...
Stat<double> calcFrechetDistance(const Trajectory & traj1, const Trajectory & traj2)
{
  ...
  for (size_t i = 0; i < traj1.points.size(); ++i) {
    for (size_t j = 0; j < traj2.points.size(); ++j) {
      const double dist = autoware_universe_utils::calcDistance2d(traj1.points[i], traj2.points[j]);
      if (i > 0 && j > 0) {
...
Stat<double> calcLateralDistance(const Trajectory & traj1, const Trajectory & traj2)
{
  ...
  for (const auto & point : traj2.points) {
    ...
    if (
      ...
      // orthogonal distance
      const auto p1 = autoware_universe_utils::getPoint(traj1.points[nearest_segment_idx]);
      const auto p2 = autoware_universe_utils::getPoint(traj1.points[nearest_segment_idx + 1]);
