...
__Note__: The _autoware_ user created in the Docker image has a specific user
and group ID. If the _autoware_ user ID does not match your host's user ID, the
shared directory inside the container will be read-only. If you need to be able
to copy data out of the container, you can create a user group on your host with
the ID number *15214* which is the same group ID used in the container. Once you
have created the group on your host, add your host user to this new group and
then change the shared directory group ownership to this new group. Example:
```bash
sudo groupadd --gid 15214 autoware
sudo usermod -aG autoware
sudo chown -R :autoware ~/shared_dir
```
You must logout then login for the group changes to take effect.
