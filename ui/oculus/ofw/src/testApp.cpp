...
void testApp::update()
{
  ...
  // JUMP
  if ((jump == 0 && OculusRPY.x > -100 && OculusRPY.x < -50) || (jump == 1 && OculusRPY.x >= -30))
    count++;
  else
    count = 0;
  if (count > 500)
  ...
  if (udp.vscan.size() == vs.getNumVertices())
  {
    ...
    {
      vs.addVertex(ofVec3f(udp.vscan[i].x - (pm.min_x + pm.max_x) / 2.0, udp.vscan[i].z,
                           -(udp.vscan[i].y - (pm.min_y + pm.max_y) / 2.0)));
    }
  }
}
...
void testApp::drawScene()
{
  ...
  {
    ofPushMatrix();
    ofTranslate(udp.oth[i].x - (pm.min_x + pm.max_x) / 2.0, udp.car_z - 1.5,
                (udp.oth[i].y - (pm.min_y + pm.max_y) / 2.0) * (-1));
    ofRotateX(90);
    ofRotateZ(-90 - euler.y);
  ...
  {
    ofDrawBox(udp.obj[i].x - (pm.min_x + pm.max_x) / 2.0, udp.obj[i].z - 1,
              (udp.obj[i].y - (pm.min_y + pm.max_y) / 2.0) * (-1), 1, 1, 1);
  }
  ofFill();
  ofPopStyle();
#endif
  ...
  while (1)
  {
    while (fs.getline(buf, BUFSIZE))
    {
      ...
      if (message != "")
      {
        ...
        if (basetime == 0)
          basetime = string2unixtime(udp[0].c_str());
