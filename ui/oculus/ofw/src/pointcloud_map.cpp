...
void PointcloudMap::sload()
{
  ...
  for (int i = 0; i < readingFileNum; i++)
  {
    ...
    printf("DONE[ %d / %d] %d\n  X [%f : %f]  Y [%f : %f]  Z [%f : %f]\n\n", i, readingFileNum, localPoints, mi_x, ma_x,
           mi_y, ma_y, mi_z, ma_z);
  ...
  for (int i = 0; i < readingFileNum; i++)
  {
    ...
    for (int j = files[i].start; j < files[i].end; j++)
    {
      ...
      if (-3 <= points[j].z && points[j].z < level * 1 - 3)
        start.addColor(ofColor::red);
      if (level * 1 - 3 <= points[j].z && points[j].z < level * 2 - 3)
        start.addColor(ofColor::orange);
      if (level * 2 - 3 <= points[j].z && points[j].z < level * 3 - 3)
        start.addColor(ofColor::yellow);
      if (level * 3 - 3 <= points[j].z && points[j].z < level * 4 - 3)
        start.addColor(ofColor::green);
      if (level * 4 - 3 <= points[j].z && points[j].z < level * 5 - 3)
        start.addColor(ofColor::blue);
      if (level * 5 - 3 <= points[j].z && points[j].z < level * 6 - 3)
        start.addColor(ofColor::navy);
      if (level * 6 - 3 <= points[j].z && points[j].z <= level * 7 - 3)
        start.addColor(ofColor::purple);
