...
int VectorMap::set_whiteline()
{
  for (int i = 0; i < whitelines.size(); i++)
  {
    ...
    if (lines[lid].blid == 0)
    {
      if (whitelines[i].color == 'W')
        wline_mesh.push_back(set_lineclass(OF_PRIMITIVE_LINE_STRIP, ofColor::white, 3, lid));
      if (whitelines[i].color == 'Y')
        wline_mesh.push_back(set_lineclass(OF_PRIMITIVE_LINE_STRIP, ofFloatColor(1.0, 0.5, 0.0), 3, lid));
    }
  }
  whitelines.clear();
  return EXIT_SUCCESS;
...
void VectorMap::draw()
{
  ...
  for (int i = 0; i < re_mesh.size(); i++)
    re_mesh[i].draw();
  for (int i = 0; i < gutter_mesh.size(); i++)
    gutter_mesh[i].draw();
  for (int i = 0; i < curb_mesh.size(); i++)
    curb_mesh[i].draw();
  for (int i = 0; i < wline_mesh.size(); i++)
    wline_mesh[i].draw();
  for (int i = 0; i < sline_mesh.size(); i++)
    sline_mesh[i].draw();
  for (int i = 0; i < zz_mesh.size(); i++)
    zz_mesh[i].draw();
  for (int i = 0; i < cw_mesh.size(); i++)
    cw_mesh[i].draw();
  for (int i = 0; i < rsm_mesh.size(); i++)
    rsm_mesh[i].draw();
  glEnable(GL_DEPTH_TEST);
  for (int i = 0; i < pd_mesh.size(); i++)
    pd_mesh[i].draw();
  for (int i = 0; i < sd_mesh.size(); i++)
    sd_mesh[i].draw();
  for (int i = 0; i < sl_line_mesh.size(); i++)
    sl_line_mesh[i].draw();
  for (int i = 0; i < sl_pole_mesh.size(); i++)
    sl_pole_mesh[i].draw();
  for (int i = 0; i < up_mesh.size(); i++)
    up_mesh[i].draw();
}
...
void VectorMap::drawWireframe()
{
  glDisable(GL_DEPTH_TEST);
  for (int i = 0; i < re_mesh.size(); i++)
    re_mesh[i].drawWireframe();
  for (int i = 0; i < gutter_mesh.size(); i++)
    gutter_mesh[i].drawWireframe();
  for (int i = 0; i < curb_mesh.size(); i++)
    curb_mesh[i].drawWireframe();
  for (int i = 0; i < wline_mesh.size(); i++)
    wline_mesh[i].drawWireframe();
  for (int i = 0; i < sline_mesh.size(); i++)
    sline_mesh[i].drawWireframe();
  for (int i = 0; i < zz_mesh.size(); i++)
    zz_mesh[i].drawWireframe();
  for (int i = 0; i < cw_mesh.size(); i++)
    cw_mesh[i].drawWireframe();
  for (int i = 0; i < rsm_mesh.size(); i++)
    rsm_mesh[i].drawWireframe();
  glEnable(GL_DEPTH_TEST);
  for (int i = 0; i < pd_mesh.size(); i++)
    pd_mesh[i].drawWireframe();
  for (int i = 0; i < sd_mesh.size(); i++)
    sd_mesh[i].drawWireframe();
  for (int i = 0; i < sl_line_mesh.size(); i++)
    sl_line_mesh[i].drawWireframe();
  for (int i = 0; i < sl_pole_mesh.size(); i++)
    sl_pole_mesh[i].drawWireframe();
  for (int i = 0; i < up_mesh.size(); i++)
    up_mesh[i].drawWireframe();
}
