...
calcYawDeviation(const float64_t & base_yaw, const float64_t & target_yaw);
 ...
 */
MOTION_COMMON_PUBLIC size_t
findNearestIndex(const Points & points, const geometry_msgs::msg::Point & point);

/**
 * @brief search the index of the point nearest to the given target with limits on the distance and
 * yaw deviation
 * @param [in] points list of points to search
...
/**
 * @brief calculate length along trajectory from seg_idx point to nearest point to p_target on
 * trajectory If seg_idx point is after that nearest point, length is negative
 ...
 */
MOTION_COMMON_PUBLIC size_t
findNearestSegmentIndex(const Points & points, const geometry_msgs::msg::Point & point);
 ...
 */
MOTION_COMMON_PUBLIC float64_t
calcSignedArcLength(const Points & points, const size_t src_idx, const size_t dst_idx);
