...
{
  lidar_transfusion::NonMaximumSuppression nms;
  lidar_transfusion::NMSParams params;
  params.search_distance_2d_ = 1.0;
  params.iou_threshold_ = 0.2;
  params.nms_type_ = lidar_transfusion::NMS_TYPE::IoU_BEV;
  params.target_class_names_ = {"CAR"};
  nms.setParameters(params);

  std::vector<lidar_transfusion::DetectedObject> input_objects(4);
  ...
  std::vector<lidar_transfusion::DetectedObject> output_objects = nms.apply(input_objects);
