...
static void current_pose_Callback(const geometry_msgs::PoseStamped::ConstPtr &input)
{
  ...
  gettimeofday(&t, NULL);
  sprintf(buf1, "%ld%ld CAR %d %f %f %f %f %f %f", t.tv_sec, t.tv_usec, 0, input->pose.position.x,
          input->pose.position.y, input->pose.position.z, roll / PI * 180, pitch / PI * 180, yaw / PI * 180);
...
{
  for (std::vector<visualization_msgs::Marker>::const_iterator item = input->markers.begin();
       item != input->markers.end(); item++)
  {
    tf::Quaternion q(item->pose.orientation.x, item->pose.orientation.y, item->pose.orientation.z,
    ...
    {
      sprintf(buf1, "%ld%ld OTH %d %f %f %f %f %f %f", t.tv_sec, t.tv_usec, 0, item->pose.position.x,
              item->pose.position.y, item->pose.position.z, roll / PI * 180, pitch / PI * 180, yaw / PI * 180);
    }
    else if (item->color.g == 1.0f)
    {
      sprintf(buf1, "%ld%ld PED %d %f %f %f %f %f %f", t.tv_sec, t.tv_usec, 0, item->pose.position.x,
              item->pose.position.y, item->pose.position.z, roll / PI * 180, pitch / PI * 180, yaw / PI * 180);
    }
    else
    {
      sprintf(buf1, "%ld%ld OBJ %d %f %f %f %f %f %f", t.tv_sec, t.tv_usec, 0, item->pose.position.x,
              item->pose.position.y, item->pose.position.z, roll / PI * 180, pitch / PI * 180, yaw / PI * 180);
    }
...
static void vscan_Callback(const sensor_msgs::PointCloud2ConstPtr &msg)
{
  ...
  for (pcl::PointCloud<pcl::PointXYZ>::const_iterator item = _vscan.begin(); item != _vscan.end(); item++)
  {
    ...
    else
    {
      ...
      gettimeofday(&t, NULL);
      sprintf(buf2, "%ld%ld VSC %d %f %f %f %f %f %f", t.tv_sec, t.tv_usec, 0, prev.x, prev.y, prev.z, item->x, item->y,
              item->z);
