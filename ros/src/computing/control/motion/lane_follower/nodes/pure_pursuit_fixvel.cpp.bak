...
double GetLookAheadDistance(int waypoint)
{
    ...
    tf::Vector3 v2(_current_path.poses[waypoint].pose.position.x, _current_path.poses[waypoint].pose.position.y, _current_path.poses[waypoint].pose.position.z);
...
geometry_msgs::PoseStamped TransformWaypoint(int i)
{
    ...
    static geometry_msgs::PoseStamped transformed_waypoint;

    //waypointを車の座標系に変換
    try {
        ros::Time now = ros::Time::now();
        tfListener.waitForTransform(_mobility_frame, _current_path.header.frame_id, now, ros::Duration(0.1));
        tfListener.transformPose(_mobility_frame, _current_path.poses[i], transformed_waypoint);
...
//            if (_transformed_waypoint.pose.position.x < 0)
            ...
            if (Distance > _lookahead_threshold) {
                std::cout << "LookAhead Threshold = " << _lookahead_threshold << std::endl;
                std::cout << "Distance = " << Distance << std::endl;
                ...
                marker.pose.position = _current_path.poses[i].pose.position;
                marker.pose.orientation = _current_path.poses[i].pose.orientation;
                marker.scale.x = 0.5;
                marker.scale.y = 0.5;
                marker.scale.z = 0.5;
                marker.color.a = 1.0;
...
geometry_msgs::Twist CalculateCmdTwist()
{
    ...
    std::cout << "current path (" << _current_path.poses[_next_waypoint].pose.position.x << " " << _current_path.poses[_next_waypoint].pose.position.y << " " << _current_path.poses[_next_waypoint].pose.position.z << ")" << std::endl;
    std::cout << "transformed_path : (" << transformed_waypoint.pose.position.x << " " << transformed_waypoint.pose.position.y << " " << transformed_waypoint.pose.position.z << ")" << std::endl;

    //std::cout << "Lookahead Distance = " << lookahead_distance << std::endl;

    double radius = 0;
    if (transformed_waypoint.pose.position.y == 0)
        radius = 0;
    else
        radius = pow(lookahead_distance, 2) / (2 * transformed_waypoint.pose.position.y);
...
int main(int argc, char **argv)
{
    ...
    //subscribe topic
    ros::Subscriber waypoint_subcscriber = nh.subscribe("lane_waypoint", 1000, WayPointCallback);
