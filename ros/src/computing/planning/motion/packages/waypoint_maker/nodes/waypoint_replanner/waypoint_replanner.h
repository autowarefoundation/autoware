...
public:
  ...
  void replanLaneWaypointVel(autoware_planning_msgs::Lane& lane);

protected:
  void changeVelSign(autoware_planning_msgs::Lane& lane, bool positive) const;
  int getDirection(const autoware_planning_msgs::Lane& lane) const;
  void resampleLaneWaypoint(const double resample_interval, autoware_planning_msgs::Lane& lane, int dir);
  void resampleOnStraight(const CbufGPoint& curve_point, autoware_planning_msgs::Lane& lane);
  void resampleOnCurve(const geometry_msgs::Point& target_point,
    const std::vector<double>& param,autoware_planning_msgs::Lane& lane, int dir);

  const CbufGPoint getCrvPointsOnResample(const autoware_planning_msgs::Lane& lane,
    const autoware_planning_msgs::Lane& original_lane, unsigned long original_index) const;
  const CbufGPoint getCrvPoints(const autoware_planning_msgs::Lane& lane, unsigned long index) const;

  void createRadiusList(const autoware_planning_msgs::Lane& lane, std::vector<double>& curve_radius);
  const double calcVelParam(double vmax) const;
  void createCurveList(const std::vector<double>& curve_radius, KeyVal& curve_list);
  void createVmaxList(const autoware_planning_msgs::Lane& lane, const KeyVal &curve_list,
  ...
  double searchVmaxByRange(unsigned long start_idx, unsigned long end_idx, unsigned int offset,
    const autoware_planning_msgs::Lane &lane) const;
  ...
  void limitVelocityByRange(unsigned long start_idx, unsigned long end_idx, unsigned int offset, double vmin,
    autoware_planning_msgs::Lane& lane);
  void limitAccelDecel(const unsigned long idx, autoware_planning_msgs::Lane& lane);

  const std::vector<double> calcCurveParam(CbufGPoint point) const;
  const double calcPathLength(const autoware_planning_msgs::Lane& lane) const;
};
}
#endif
