...
void PurePursuitNode::run()
{
  ...
  while (ros::ok())
  {
    ...
    node_status_publisher_ptr_->CHECK_RATE("topic_vehicle_cmd_slow_in_pure_pursuit",8,5,1,"topic vehicle_cmd publish rate in pure_pursuit node is slow.");
...
void PurePursuitNode::publishTwistStamped(const bool &can_get_curvature, const double &kappa) const
{
  ...
  node_status_publisher_ptr_->CHECK_MAX_VALUE("twist_cmd_linear_high",ts.twist.linear.x,2.2,3.3,4.4,"linear twist_cmd is too high");
  pub1_.publish(ts);
}
