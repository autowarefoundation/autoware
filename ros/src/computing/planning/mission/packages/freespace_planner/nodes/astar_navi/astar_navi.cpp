...
void mapCallback(const nav_msgs::OccupancyGridConstPtr &msg)
{
  ...
  tf::Quaternion quat(msg->info.origin.orientation.x, msg->info.origin.orientation.y, msg->info.origin.orientation.z,
  ...
  for (i = 0; i < SIZE_Y; i++)
  {
    for (j = 0; j < SIZE_X; j++)
    {
      if (msg->data[SIZE_Y * i + j] == 100)
      {  // obstacle
        map[i][j][0].status = OBS;
      }
      else if (msg->data[SIZE_Y * i + j] == -1)
      {  // unknown
        map[i][j][0].status = OBS;
      }
      else
...
void GoalCallback(const geometry_msgs::PoseStampedConstPtr &msg)
{
  ...
  tf::Quaternion quat(msg->pose.orientation.x, msg->pose.orientation.y, msg->pose.orientation.z,
...
void StartCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr &msg)
{
  ...
  map[sy][sx][stheta].x = dsx;
  map[sy][sx][stheta].y = dsy;
  ...
  current_x = map[sy][sx][stheta].x;
  current_y = map[sy][sx][stheta].y;
    ...
    for (i = 0; i < 8; i++)
    {
      ...
      {  // out of range
        continue;
               ...
               WaveFrontCollide(next_x, next_y))
      {  // collide obstacles
        continue;
      }
      else
...
bool Collide(const double &car_x, const double &car_y, const double &car_theta)
{
  ...
  for (i = grid_y - size_y; i < grid_y + size_y; i++)
  {
    for (j = grid_x - size_x; j < grid_x + size_x; j++)
    {
      ...
      x2 += (double)grid_x + x_offset;
      y2 += (double)grid_y + y_offset;
      grid_x2 = (int)x2;
      grid_y2 = (int)y2;
      // std::cout << "(" << x2 << ", " << y2 << ")" << std::endl;
      if (grid_x2 < 0 || grid_y2 < 0 || grid_x2 >= SIZE_X || grid_y2 >= SIZE_Y)
      {
        return true;
      }
      if (map[grid_y2][grid_x2][0].status == OBS)
      {
        return true;
      }
    }
...
void CalcCarsize(const double &car_x, const double &car_y, const double &car_theta)
{
  ...
  for (i = grid_y - size_y; i < grid_y + size_y; i++)
  {
    for (j = grid_x - size_x; j < grid_x + size_x; j++)
    {
      if ((i == grid_y - size_y && j == grid_x - size_x) || (i == grid_y - size_y && j == grid_x + size_x - 1) ||
          (i == grid_y + size_y - 1 && j == grid_x - size_x) ||
          (i == grid_y + size_y - 1 && j == grid_x + size_x - 1) ||
          (i == grid_y - size_y && j == grid_x + size_x - 5) || (i == grid_y + size_y - 1 && j == grid_x + size_x - 5))
      {
        //
        x2 = (double)(j - grid_x) * cos(car_theta) - (double)(i - grid_y) * sin(car_theta);
        y2 = (double)(j - grid_x) * sin(car_theta) + (double)(i - grid_y) * cos(car_theta);
...
int AStar()
{
  ...
  tfpose.position.x = current_x;
  tfpose.position.y = current_y;
  tfpose.position.z = 0.0;
  tf::Vector3 tfpoint = TransformPoint(tfpose);

  /* current grid cell */
  norm_x = (int)(tfpoint.getX() * 10);
  norm_y = (int)(tfpoint.getY() * 10);
  ...
  for (i = 0; i < 6; i++)
  {
    switch (i)
    {
      case 0:  // forward //
        // continue;
        if (map[norm_y][norm_x][norm_theta].steering == 1)
        {
          continue;
        }
      ...
      case 1:  // backward //
        // continue;
        if (map[norm_y][norm_x][norm_theta].steering == 0)
        {
          continue;
        }
      ...
      case 2:  // left forward //
        // continue;
        if (map[norm_y][norm_x][norm_theta].steering == 3 || map[norm_y][norm_x][norm_theta].steering == 4)
        {
          continue;
        }
      ...
      case 3:  // left backward //
        // continue;
        if (map[norm_y][norm_x][norm_theta].steering == 2 || map[norm_y][norm_x][norm_theta].steering == 5)
        {
          continue;
        }
        temp.x = (ARC / THETA) * cos(current_theta + 3 * M_PI / 2 - THETA) + center_x2;
      ...
      case 4:  // right forward //
        // continue;
        if (map[norm_y][norm_x][norm_theta].steering == 5 || map[norm_y][norm_x][norm_theta].steering == 2)
        {
          continue;
        }
      ...
      case 5:  // right backward //
        // continue;
        if (map[norm_y][norm_x][norm_theta].steering == 4 || map[norm_y][norm_x][norm_theta].steering == 3)
        {
          continue;
        }
        temp.x = (ARC / THETA) * cos(current_theta + M_PI / 2 + THETA) + center_x1;
    ...
    tftheta = ChangeTheta(tftheta);
    if (tftheta < 0 || tftheta >= 2 * M_PI)
    {
      std::cout << "ERROR!!" << std::endl;
    }  ///
    tfpose.position.x = temp.x;
    tfpose.position.y = temp.y;
    tfpose.position.z = 0.0;
    tfpoint = TransformPoint(tfpose);

    grid_x = (int)(tfpoint.getX() * 10);
    grid_y = (int)(tfpoint.getY() * 10);
    ...
    else if (map[grid_y][grid_x][grid_theta].goal && GoalAngle(temp.theta))
    ...
    {
      if (map[norm_y][norm_x][norm_theta].gc + cost < map[grid_y][grid_x][grid_theta].gc)
      {
        map[grid_y][grid_x][grid_theta].x = temp.x;
        map[grid_y][grid_x][grid_theta].y = temp.y;
        map[grid_y][grid_x][grid_theta].theta = temp.theta;
