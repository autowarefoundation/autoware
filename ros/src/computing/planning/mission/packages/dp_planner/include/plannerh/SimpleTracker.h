...
public:
	...
	KFTrack(double x, double y, double a, long id)
	{
		...
		m_filter.transitionMatrix = *(cv::Mat_<float>(4, 4) << 1,0,1,0,
															   0,1,0,1,
															   0,0,1,0,
															   0,0,0,1);
		...
		cv::setIdentity(m_filter.errorCovPost, cv::Scalar::all(.0));
		m_filter.predict();
	}
...
////		prev_v = v;
	...
	void UpdateTrackingFullKalman(const double& x, const double& y, const double& a, double& x_new, double & y_new , double& a_new, double& v)
	{
		cv::Mat_<float> measurement(2,1);
		cv::Mat_<float> prediction(4,1);
		measurement(0) = x;
...
public:
	...
	KFTrackV(double x, double y, double a, long id)
	{
		dt = 0.01;
		prev_x = x;
		prev_y = y;
