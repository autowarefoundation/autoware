...
void PlannerSelector::initROS()
{
  ...
  Pubs["final_waypoints"] = nh_.advertise<autoware_planning_msgs::Lane>("/final_waypoints", 1);
...
void PlannerSelector::callbackFromLattice(const std_msgs::Int32 &msg)
{
  ...
  try
  {
    autoware_planning_msgs::Waypoint dp_point = final_waypoints_dp_.waypoints.at(config_waypoints_num_);
    autoware_planning_msgs::Waypoint astar_point = final_waypoints_astar_.waypoints.at(config_waypoints_num_);
...
void PlannerSelector::callbackFromWaypoints(const ros::MessageEvent<autoware_planning_msgs::Lane const> &event)
{
  const ros::M_string &header = event.getConnectionHeader();
  std::string topic = header.at("topic");
  const autoware_planning_msgs::Lane *waypoints = event.getMessage().get();
