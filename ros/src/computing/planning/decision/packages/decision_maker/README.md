...
VehicleReady|-|Vehicle is ready to move.|Exits to VehicleEmergency when `emergency` key is given by state_cmd from other states, or if `emergency_flag` is set true by other states. <br> Or transits to BatteryCharging when `charging_start` key by state_cmd from other nodes(e.g. by AMS).
