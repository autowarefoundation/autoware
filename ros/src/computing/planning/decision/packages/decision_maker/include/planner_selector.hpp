...
private:
  ...
  std::unordered_map<std::string, autoware_planning_msgs::Lane> waypoints_;
  std::unordered_map<std::string, ros::Publisher> Pubs;
  std::unordered_map<std::string, ros::Subscriber> Subs;

  autoware_planning_msgs::Lane final_waypoints_dp_;
  autoware_planning_msgs::Lane final_waypoints_astar_;
...
public:
  ...
  void callbackFromWaypoints(const ros::MessageEvent<autoware_planning_msgs::Lane const> &event);
