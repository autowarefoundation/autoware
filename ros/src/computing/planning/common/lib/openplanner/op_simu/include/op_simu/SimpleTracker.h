...
//		errorSmoother.result.x = 0;
  ...
  void UpdateTracking(
    double _dt, const PlannerHNS::DetectedObject & oldObj,
    PlannerHNS::DetectedObject & predObj)
  {
    ...
    if (predObj.centers_list.size() > 1) {
      double diff_y = predObj.center.pos.y -
        predObj.centers_list.at(predObj.centers_list.size() - 1).pos.y;
      double diff_x = predObj.center.pos.x -
    ...
    if (predObj.centers_list.size() > 3) {
      ...
      predObj.center.pos.a =
        ...
        predObj.centers_list.at(predObj.centers_list.size() - 3).pos.a) / 3.0;
    } else if (predObj.centers_list.size() > 2) {
      predObj.center.pos.a =
        (predObj.centers_list.at(predObj.centers_list.size() - 1).pos.a +
        predObj.centers_list.at(predObj.centers_list.size() - 2).pos.a) / 2.0;
...
public:
  ...
  CostRecordSet(
    int obj_index, int track_index, double _distance_diff, double _size_diff,
...
private:
  ...
  int InsidePolygon(
    const std::vector<PlannerHNS::GPSPoint> & polygon,
