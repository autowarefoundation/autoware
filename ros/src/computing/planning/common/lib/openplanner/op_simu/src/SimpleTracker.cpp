...
void SimpleTracker::InitializeInterestRegions(std::vector<InterestCircle *> & regions)
{
  ...
  while (1) {
    ...
    if (regions.size() == 0) {
      ...
      std::cout << "Region No: " << regions.size() << ", Radius: " << pCir->radius <<
  ...
  const TRACKING_TYPE & type)
{
  if (!m_bFirstCall) {
    ...
    m_StateDiff.pos.a = op_utility_ns::UtilityH::AngleBetweenTwoAnglesPositive(currPose.pos.a,
        m_PrevState.pos.a) *
      op_utility_ns::UtilityH::GetSign(m_PrevState.pos.a - currPose.pos.a);
...
void SimpleTracker::MatchClosest()
{
  ...
  while (m_DetectedObjects.size() > 0) {
    ...
    for (unsigned int jj = 0; jj < m_DetectedObjects.size(); jj++) {
      double object_size = sqrt(m_DetectedObjects.at(jj).w * m_DetectedObjects.at(
            jj).w + m_DetectedObjects.at(jj).l * m_DetectedObjects.at(jj).l + m_DetectedObjects.at(
            jj).h * m_DetectedObjects.at(jj).h);

      for (unsigned int i = 0; i < m_TrackSimply.size(); i++) {
        double d = hypot(m_DetectedObjects.at(jj).center.pos.y - m_TrackSimply.at(
              i).obj.center.pos.y, m_DetectedObjects.at(jj).center.pos.x - m_TrackSimply.at(
              i).obj.center.pos.x);
        double old_size = sqrt(m_TrackSimply.at(i).obj.w * m_TrackSimply.at(
              i).obj.w + m_TrackSimply.at(i).obj.l * m_TrackSimply.at(i).obj.l + m_TrackSimply.at(
              i).obj.h * m_TrackSimply.at(i).obj.h);
    ...
    {
      std::cout << "MatchObj: " << m_TrackSimply.at(iCloseset_track).obj.id << ", MinD: " <<
          dCloseset << ", SizeDiff: (" << size_diff << ")" << ", ObjI" << iCloseset_obj <<
          ", TrackI: " << iCloseset_track << ", CMatch: " << bFoundMatch << std::endl;
      ...
      if (iCloseset_obj != -1) {
        ...
        KFTrackV track(m_DetectedObjects.at(iCloseset_obj).center.pos.x, m_DetectedObjects.at(
            iCloseset_obj).center.pos.y, m_DetectedObjects.at(
            iCloseset_obj).actual_yaw, m_DetectedObjects.at(
            iCloseset_obj).id, m_dt, m_nMinTrustAppearances);
        ...
        KFTrackV track(m_DetectedObjects.at(0).center.pos.x, m_DetectedObjects.at(
            0).center.pos.y, m_DetectedObjects.at(0).actual_yaw, m_DetectedObjects.at(
            0).id, m_dt, m_nMinTrustAppearances);
      ...
      for (unsigned int i = 0; i < m_TrackSimply.size(); i++) {
        ...
        if (m_TrackSimply.at(i).obj.bDirection) {
          ...
          if (hypot(diff_y, diff_x) > 0.2) {
            ...
            a_diff = op_utility_ns::UtilityH::AngleBetweenTwoAnglesPositive(a, m_TrackSimply.at(
                  i).obj.center.pos.a);
...
//			}
      ...
      min_set.angle_diff < m_MAX_ASSOCIATION_ANGLE_DIFF)
    {
      ...
      if (min_set.i_obj != -1) {
        ...
        KFTrackV track(m_DetectedObjects.at(min_set.i_obj).center.pos.x, m_DetectedObjects.at(
            min_set.i_obj).center.pos.y, m_DetectedObjects.at(
            min_set.i_obj).actual_yaw, m_DetectedObjects.at(
            min_set.i_obj).id, m_dt, m_nMinTrustAppearances);
...
void SimpleTracker::AssociateAndTrack()
{
  ...
  while (m_DetectedObjects.size() > 0) {
    ...
    bool bFoundMatch = false;
    double min_size = -1;
    std::cout << "DetObjSize: " << m_DetectedObjects.size() << ", TracksSize: " <<
        m_TrackSimply.size() << std::endl;
    for (unsigned int jj = 0; jj < m_DetectedObjects.size(); jj++) {
      ...
      for (unsigned int i = 0; i < m_TrackSimply.size(); i++) {
        ...
        std::cout << "Test: " << m_TrackSimply.at(i).obj.id << ", MinD: " << d << ", ObjS: " <<
            object_size << ", ObjI: " << jj << ", TrackS: " << old_size << ", TrackI: " << i <<
            std::endl;
    ...
    {
      std::cout << "MatchObj: " << m_TrackSimply.at(iCloseset_track).obj.id << ", MinD: " <<
          dCloseset << ", MinS: " << min_size << ", ObjI" << iCloseset_obj << ", TrackI: " <<
          iCloseset_track << ", ContourMatch: " << bFoundMatch << std::endl;
      m_DetectedObjects.at(iCloseset_obj).id = m_TrackSimply.at(iCloseset_track).obj.id;
      MergeObjectAndTrack(m_TrackSimply.at(iCloseset_track), m_DetectedObjects.at(iCloseset_obj));
      ...
      if (iCloseset_obj != -1) {
        ...
        std::cout << "UnMachedObj: " << iTracksNumber << ", MinD: " << dCloseset << ", MinS: " <<
            min_size << ", ObjI:" << iCloseset_obj << ", TrackI: " << iCloseset_track <<
        ...
        AssociateToRegions(track);
        m_TrackSimply.push_back(track);
        std::cout << "NewObj: " << iTracksNumber << ", MinD: " << dCloseset << ", MinS: " <<
            min_size << ", ObjI:" << 0 << ", TrackI: " << iCloseset_track << ", ContMatch: " <<
