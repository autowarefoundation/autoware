...
void TrajectoryFollower::Init(
  const ControllerParams & params, const CAR_BASIC_INFO & vehicleInfo,
...
void TrajectoryFollower::PrepareNextWaypoint(
  const PlannerHNS::WayPoint & CurPos,
  ...
  const double & currSteering)
{
  ...
  for (unsigned int i = 0; i < nIterations; i++) {
    PredictMotion(m_ForwardSimulation.pos.x, m_ForwardSimulation.pos.y, m_ForwardSimulation.pos.a,
      currSteering, currVelocity, m_VehicleInfo.wheel_base, 0.01);
  }
...
void TrajectoryFollower::UpdateCurrentPath(const std::vector<PlannerHNS::WayPoint> & path)
{
  ...
  if (path.size() > 0) {
    m_WayPointsDensity = hypot(path.at(1).pos.y - path.at(0).pos.y, path.at(1).pos.x - path.at(
          0).pos.x);
  }
  ...
  const PlannerHNS::BehaviorState & CurrBehavior, double & desiredSteerAngle)
{
  ...
  if (CurrBehavior.state == FORWARD_STATE || CurrBehavior.state == TRAFFIC_LIGHT_STOP_STATE ||
    CurrBehavior.state == STOP_SIGN_STOP_STATE || CurrBehavior.state == FOLLOW_STATE)
  {
...
void TrajectoryFollower::PredictMotion(
  ...
  PlannerHNS::SHIFT_POS & desiredShift)
{
    ...
    CurrBehavior.state == OBSTACLE_AVOIDANCE_STATE)
  {
    ...
    double inv_time = 2.0 *
      ((CurrBehavior.followDistance - critical_long_front_distance) - CurrStatus.speed);
    if (inv_time == 0) {
...
void TrajectoryFollower::CalibrationStep(const double & dt,
  const PlannerHNS::VehicleState & CurrStatus,
  double & desiredSteer, double & desiredVelocity)
{
  ...
  cout << "i:" << m_iNextTest << ", desVel:" << desiredVelocity << ", CurVel:" <<
      CurrStatus.speed <<
...
void TrajectoryFollower::LogCalibrationData(const PlannerHNS::VehicleState & currState,
