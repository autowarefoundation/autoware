...
void ROSHelpers::GetTransformFromTF(
...
visualization_msgs::Marker ROSHelpers::CreateGenMarker(
  const double & x, const double & y, const double & z, const double & a,
  const double & r, const double & g, const double & b, const double & scale, const int & id,
  ...
  visualization_msgs::MarkerArray & trajectories)
{
  ...
  for (int i = 0; i < nMarkers; i++) {
    visualization_msgs::Marker mkr = CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "CenterMarker",
        visualization_msgs::Marker::SPHERE);
    centers.markers.push_back(mkr);
  }

  for (int i = nMarkers; i < nMarkers * 2; i++) {
    visualization_msgs::Marker mkr = CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "Directions",
        visualization_msgs::Marker::ARROW);
    dirs.markers.push_back(mkr);
  }

  for (int i = nMarkers * 2; i < nMarkers * 3; i++) {
    visualization_msgs::Marker mkr = CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "InfoText",
        visualization_msgs::Marker::TEXT_VIEW_FACING);
    text_info.markers.push_back(mkr);
  }

  for (int i = nMarkers * 3; i < nMarkers * 4; i++) {
    visualization_msgs::Marker mkr = CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "detected_polygons",
        visualization_msgs::Marker::LINE_STRIP);
    polygons.markers.push_back(mkr);
  }

  for (int i = nMarkers * 4; i < nMarkers * 5; i++) {
    visualization_msgs::Marker mkr = CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i,
...
void ROSHelpers::ConvertMatchingMarkers(
  const std::vector<std::pair<PlannerHNS::WayPoint, PlannerHNS::WayPoint>> & match_list,
  visualization_msgs::MarkerArray & tracked_traj_d, visualization_msgs::MarkerArray & tracked_traj,
  int start_id)
{
  ...
  for (unsigned int i = 0; i < match_list.size(); i++) {
    visualization_msgs::Marker match_mkr = CreateGenMarker(0, 0, 0, 0, 1, 0, 0, 0.2, start_id + i,
        "matching_connections",
        visualization_msgs::Marker::LINE_STRIP);
  ...
  visualization_msgs::MarkerArray & tracked_traj)
{
  ...
  for (unsigned int i = 0; i < trackedObstacles.size(); i++) {
    ...
    //Update Stage
    visualization_msgs::Marker center_mkr = CreateGenMarker(trackedObstacles.at(
          i).center.pos.x, trackedObstacles.at(i).center.pos.y, trackedObstacles.at(i).center.pos.z,
        trackedObstacles.at(
          i).center.pos.a, 1, 0, 0, 0.5, i, "CenterMarker", visualization_msgs::Marker::SPHERE);
    ...
    if (trackedObstacles.at(i).bDirection) {
      visualization_msgs::Marker dir_mkr = CreateGenMarker(trackedObstacles.at(
            i).center.pos.x, trackedObstacles.at(i).center.pos.y, trackedObstacles.at(
            i).center.pos.z + 0.5,
          trackedObstacles.at(i).center.pos.a, 0, 1, 0, 0.3,
          centers.markers.size() + i, "Directions", visualization_msgs::Marker::ARROW);
      dir_mkr.scale.x = 0.4;
...
//		else
    text_mkr = CreateGenMarker(trackedObstacles.at(i).center.pos.x + 0.5, trackedObstacles.at(
          i).center.pos.y + 0.5, trackedObstacles.at(i).center.pos.z + 1,
        trackedObstacles.at(i).center.pos.a, 1, 1, 1, 1.2,
        centers.markers.size() * 2 + i, "InfoText", visualization_msgs::Marker::TEXT_VIEW_FACING);
    ...
    //Polygons
    visualization_msgs::Marker poly_mkr = CreateGenMarker(0, 0, 0, 0, 1, 0.25, 0.25, 0.1,
        centers.markers.size() * 3 + i, "detected_polygons",
        visualization_msgs::Marker::LINE_STRIP);
    ...
    //Trajectories
    visualization_msgs::Marker traj_mkr = CreateGenMarker(0, 0, 0, 0, 1, 1, 0, 0.1,
        centers.markers.size() * 4 + i, "tracked_trajectories",
        visualization_msgs::Marker::LINE_STRIP);
...
void ROSHelpers::CreateCircleMarker(
  const PlannerHNS::WayPoint & _center, const double & radius,
  const int & start_id,
  visualization_msgs::Marker & circle_points)
{
  circle_points = CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 0.2, start_id, "Detection_Circles",
      visualization_msgs::Marker::LINE_STRIP);
...
//		visualization_msgs::Marker path_mkr = CreateGenMarker(0,0,0,0,r,g,b,0.1,i,"Predicted_Trajectories", visualization_msgs::Marker::LINE_STRIP);
    ...
    visualization_msgs::Marker path_mkr = CreateGenMarker(0, 0, 0, 0, 1.0 * prop, 0.1 * prop,
        0.1 * prop, 0.1, i,
        "Predicted_Trajectories",
        visualization_msgs::Marker::LINE_STRIP);
  ...
  visualization_msgs::MarkerArray & curbs_markers_d)
{
  ...
  for (unsigned int i = 0; i < curbs.size(); i++) {
    if (curbs.at(i).contour.size() > 0) {
      visualization_msgs::Marker curb_mkr = CreateGenMarker(curbs.at(i).contour.at(0).x, curbs.at(
            i).contour.at(0).y, curbs.at(i).contour.at(
            0).z, 0, 1, 0.54, 0, 0.2, i, "map_detected_curbs", visualization_msgs::Marker::SPHERE);
  ...
  visualization_msgs::MarkerArray & collision_markers_d)
{
  ...
  for (unsigned int i = 0; i < col_points.size(); i++) {
    visualization_msgs::Marker mkr = CreateGenMarker(col_points.at(i).pos.x, col_points.at(
          i).pos.y, col_points.at(
          i).pos.z, 0, 1, 0, 0, 0.5, i, "collision_points_rviz",
        visualization_msgs::Marker::SPHERE);
...
void ROSHelpers::ConvertFromRoadNetworkToAutowareVisualizeMapFormat(
  const PlannerHNS::RoadNetwork & map, visualization_msgs::MarkerArray & markerArray)
{
  ...
  visualization_msgs::MarkerArray & paths)
{
  paths.markers.clear();
  for (int i = 0; i < nMarkers; i++) {
    visualization_msgs::Marker mkr = CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 0.05, i, "Particles",
        visualization_msgs::Marker::ARROW);
    mkr.scale.x = 0.3;
    paths.markers.push_back(mkr);
  }
  ...
  visualization_msgs::MarkerArray & part_markers_d)
{
  ...
  for (unsigned int i = 0; i < points.size(); i++) {
    ...
    if (points.at(i).bDir == PlannerHNS::STANDSTILL_DIR) {
      mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(
            i).pos.a, 1, 0, 0, 0.05, i, "Particles", visualization_msgs::Marker::ARROW);
    } else if (points.at(i).bDir == PlannerHNS::FORWARD_DIR) {
      mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(
            i).pos.a, 1, 1, 1, 0.05, i, "Particles", visualization_msgs::Marker::ARROW);
    } else if (points.at(i).bDir == PlannerHNS::FORWARD_RIGHT_DIR) {
      mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(
            i).pos.a, 0, 1, 0, 0.05, i, "Particles", visualization_msgs::Marker::ARROW);
    } else if (points.at(i).bDir == PlannerHNS::FORWARD_LEFT_DIR) {
      mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(
            i).pos.a, 0, 0, 1, 0.05, i, "Particles", visualization_msgs::Marker::ARROW);
    } else if (points.at(i).bDir == PlannerHNS::BACKWARD_DIR) {
      mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(
            i).pos.a, 1, 0, 1, 0.05, i, "Particles", visualization_msgs::Marker::ARROW);
    } else {
      mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(
            i).pos.a, 1, 1, 0, 0.05, i, "Particles", visualization_msgs::Marker::ARROW);
    }
...
void ROSHelpers::TrajectoriesToColoredMarkers(
  ...
  const std::vector<PlannerHNS::TrajectoryCost> & traj_costs, const int & iClosest,
  visualization_msgs::MarkerArray & markerArray)
{
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
  ...
  for (unsigned int il = 0; il < paths.size(); il++) {
    for (unsigned int i = 0; i < paths.at(il).size(); i++) {
      ...
      if (localPlanner.m_TrajectoryCostsCalculatotor.m_TrajectoryCosts.size() == paths.size()) {
        float norm_cost = localPlanner.m_TrajectoryCostsCalculatotor.m_TrajectoryCosts.at(i).cost *
          paths.size();
        if (norm_cost <= 1.0) {
          lane_waypoint_marker.color.r = norm_cost;
          lane_waypoint_marker.color.g = 1.0;
...
void ROSHelpers::ConvertFromAutowareCloudClusterObstaclesToPlannerH(
  ...
  const int & n_poly_quarters, const double & poly_resolution, int & nOriginalPoints,
  int & nContourPoints)
{
  ...
  for (unsigned int i = 0; i < clusters.clusters.size(); i++) {
    ...
    if (obj.distance_to_center > detection_radius || object_size < min_obj_size ||
      object_size > max_obj_size)
    {
      continue;
    }
...
PlannerHNS::AutowareBehaviorState ROSHelpers::ConvertBehaviorStateFromPlannerHToAutoware(
  const PlannerHNS::BehaviorState & beh)
{
  ...
  visualization_msgs::MarkerArray & markerArray)
{
  ...
  for (unsigned int i = 0; i < lane_waypoints_array.lanes.size(); i++) {
    ...
    for (unsigned int j = 0; j < lane_waypoints_array.lanes.at(i).waypoints.size(); j++) {
      ...
      geometry_msgs::Point relative_p;
      relative_p.y = 0.5;
      velocity_marker.pose.position = calcAbsoluteCoordinate(relative_p, lane_waypoints_array.lanes.at(
            i).waypoints.at(j).pose.pose);
  ...
  visualization_msgs::MarkerArray & markerArray)
{
  ...
  for (unsigned int i = 0; i < lights.size(); i++) {
    if (lights.at(i).lightState == RED_LIGHT) {
      visualization_msgs::Marker mkr = CreateGenMarker(lights.at(i).pos.x, lights.at(
            i).pos.y, lights.at(
            i).pos.z, 0, 1, 0, 0, 3, i, "traffic_light_visualize",
          visualization_msgs::Marker::SPHERE);
      markerArray.markers.push_back(mkr);
    } else if (lights.at(i).lightState == GREEN_LIGHT) {
      visualization_msgs::Marker mkr = CreateGenMarker(lights.at(i).pos.x, lights.at(
            i).pos.y, lights.at(
            i).pos.z, 0, 0, 1, 0, 3, i, "traffic_light_visualize",
          visualization_msgs::Marker::SPHERE);
      markerArray.markers.push_back(mkr);
    }
  }
...
void ROSHelpers::ConvertFromAutowareDetectedObjectToOpenPlannerDetectedObject(
  const autoware_msgs::DetectedObject & det_obj, PlannerHNS::DetectedObject & obj)
{
  obj.id = det_obj.id;
  ...
  for (unsigned int j = 0; j < det_obj.candidate_trajectories.lanes.size(); j++) {
    ...
    PlannerHNS::ROSHelpers::ConvertFromAutowareLaneToLocalLane(det_obj.candidate_trajectories.lanes.at(
        j), _traj);
  ...
  autoware_msgs::DetectedObject & obj)
{
  ...
  for (unsigned int j = 0; j < det_obj.predTrajectories.size(); j++) {
    ...
    PlannerHNS::ROSHelpers::ConvertFromLocalLaneToAutowareLane(det_obj.predTrajectories.at(
        j), pred_traj);
...
void ROSHelpers::UpdateRoadMap(
  const AutowareRoadNetwork & src_map,
...
//		l.RoadSecID =  src_map.lanes.at(i).;
  ...
  PlannerHNS::MappingHelpers::ConstructRoadNetworkFromROSMessage(lanes, points, dts, inters, areas,
    line_data, stop_line_data,
    signal_data, vector_data,
    curb_data, roadedge_data, way_area,
    crossing, nodes_data, conn_data,
    origin, out_map);
}
...
void ROSHelpers::GetIndicatorArrows(
  ...
  const double & length,
  const PlannerHNS::LIGHT_INDICATOR & indicator, const int & id,
  visualization_msgs::MarkerArray & markerArray)
{
  ...
  visualization_msgs::Marker mkr_l = PlannerHNS::ROSHelpers::CreateGenMarker(top_left.x, top_left.y,
      top_left.z, top_left.a,
      color_l.r, color_l.g,
      color_l.b, 1.0, id,
  ...
  mkr_l.scale.y = 0.4;
  mkr_l.scale.z = 0.4;
  visualization_msgs::Marker mkr_r = PlannerHNS::ROSHelpers::CreateGenMarker(top_right.x,
      top_right.y,
      top_right.z,
      top_right.a, color_r.r,
      color_r.g, color_r.b,
      1.0, id,
      "simu_car_indicator_right",
      visualization_msgs::Marker::ARROW);
  mkr_r.scale.y = 0.4;
  mkr_r.scale.z = 0.4;
