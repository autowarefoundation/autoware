...
bool PlanningHelpers::GetRelativeInfoRange(
  const std::vector<std::vector<WayPoint>> & trajectories,
  ...
  for (unsigned int i = 0; i < infos.size(); i++) {
    if (searchDistance > 0) {
      double laneChangeCost =
        trajectories.at(infos.at(i).iGlobalPath).at(infos.at(i).iFront).laneChangeCost;
...
bool PlanningHelpers::GetRelativeInfo(
...
//			cout << " Alert Alert !!!! fast: " << info.iFront << ", slow: " << old_index  << endl;
    ...
    if (info.iFront == 0) {
      ...
      p0 = trajectory.at(info.iFront - 1);
      p1 =
        WayPoint((p0.pos.x + trajectory.at(info.iFront).pos.x) / 2.0,
          (p0.pos.y + trajectory.at(info.iFront).pos.y) / 2.0,
          (p0.pos.z + trajectory.at(info.iFront).pos.z) / 2.0, p0.pos.a);
    }
  }
  ...
  const int & prevIndex)
{
  ...
  if (trajectory.size() == 2) {
    ...
    if (info.iFront == 0) {
      ...
      p0 = _trajectory.at(info.iFront - 1);
      p1 =
        WayPoint((p0.pos.x + _trajectory.at(info.iFront).pos.x) / 2.0,
          (p0.pos.y + _trajectory.at(info.iFront).pos.y) / 2.0,
          (p0.pos.z + _trajectory.at(info.iFront).pos.z) / 2.0, p0.pos.a);
    }
...
bool PlanningHelpers::GetThreePointsInfo(
  const WayPoint & p0, const WayPoint & p1,
  const WayPoint & p2, WayPoint & perp_p, double & long_d,
  double lat_d)
{
  ...
  unsigned int & point_index)
{
  ...
  //condition 2, if far after the last point on the trajectory
    ...
    while (local_i < (int)trajectory.size() - 1 && d < distance) {
      local_i++;
      d +=
        hypot(trajectory.at(local_i).pos.y - trajectory.at(local_i - 1).pos.y, trajectory.at(
            local_i).pos.x - trajectory.at(local_i - 1).pos.x);
    }
  ...
  const RelativeInfo & p2)
{
  ...
  if (p2.iFront == p1.iFront && p2.iBack == p1.iBack) {
    ...
    for (int i = p1.iFront; i < p2.iBack; i++) {
      d_on_path += hypot(trajectory.at(i + 1).pos.y - trajectory.at(i).pos.y, trajectory.at(
            i + 1).pos.x - trajectory.at(i).pos.x);
    }
...
int PlanningHelpers::GetClosestNextPointIndex_obsolete(
  const vector<WayPoint> & trajectory,
  const WayPoint & p, const int & prevIndex)
{
  ...
  const WayPoint & p, const int & prevIndex)
{
  ...
  if (iStart > 0 && iStart < size - 1 &&
    GetThreePointsInfo(trajectory.at(0), trajectory.at(iStart), p, perp_p, long_d, lat_d))
  {
  ...
  if (iEnd >= 0 && iEnd < size - 2 &&
    GetThreePointsInfo(trajectory.at(size - 1), trajectory.at(iEnd), p, perp_p, long_d, lat_d))
  {
  ...
  const WayPoint & p, const int & prevIndex)
{
  ...
  for (int i = 0; i < size; i += skip) {
    if (i + skip / 2 < size) {
      d =
        (distance2pointsSqr(trajectory[i].pos,
        p.pos) + distance2pointsSqr(trajectory[i + skip / 2].pos, p.pos)) / 2.0;
    } else {
      d = distance2pointsSqr(trajectory[i].pos, p.pos);
    }
    if (d < minD) {
  ...
  if ((size - skip / 2 - 1) > 0) {
    d =
      (distance2pointsSqr(trajectory[size - 1].pos,
      p.pos) + distance2pointsSqr(trajectory[size - skip / 2 - 1].pos, p.pos)) / 2.0;
  } else {
...
double PlanningHelpers::GetPerpDistanceToTrajectorySimple_obsolete(
  const vector<WayPoint> & trajectory, const WayPoint & p, const int & prevIndex)
{
...
bool PlanningHelpers::CompareTrajectories(
  const std::vector<WayPoint> & path1,
  const std::vector<WayPoint> & path2)
{
  ...
  for (unsigned int i = 0; i < path1.size(); i++) {
    if (path1.at(i).v != path2.at(i).v || path1.at(i).pos.x != path2.at(i).pos.x ||
      path1.at(i).pos.y != path2.at(i).pos.y || path1.at(i).pos.alt != path2.at(i).pos.alt ||
...
void PlanningHelpers::CreateManualBranch(
  std::vector<WayPoint> & path, const int & degree,
  const DIRECTION_TYPE & direction)
{
...
void PlanningHelpers::FixPathDensity(vector<WayPoint> & path, const double & distanceDensity)
{
  ...
  for (unsigned int si = 0, ei = 1; ei < path.size(); ) {
    d +=
      hypot(path.at(ei).pos.x - path.at(ei - 1).pos.x,
        path.at(ei).pos.y - path.at(ei - 1).pos.y) + remaining;
...
void PlanningHelpers::SmoothPath(
  vector<WayPoint> & path, double weight_data,
  ...
  const double & minDist)
{
  ...
  for (unsigned int i = info.iFront; i < path.size(); i++) {
    total_distance +=
      hypot(path.at(i).pos.x - path.at(i - 1).pos.x, path.at(i).pos.y - path.at(i - 1).pos.y);
...
double PlanningHelpers::CalcAngleAndCost(
  vector<WayPoint> & path, const double & lastCost,
  const bool & bSmooth)
{
...
double PlanningHelpers::CalcAngleAndCostAndCurvatureAnd2D(
...
double PlanningHelpers::CalcCircle(
  const GPSPoint & pt1, const GPSPoint & pt2, const GPSPoint & pt3,
  GPSPoint & center)
{
  ...
  center.x =
    (aSlope * bSlope * (pt1.y - pt3.y) + bSlope * (pt1.x + pt2.x) - aSlope * (pt2.x + pt3.x) ) /
    (2.0 * (bSlope - aSlope) );
...
void PlanningHelpers::ExtractPartFromPointToDistance(
  const vector<WayPoint> & originalPath, const WayPoint & pos, const double & minDistance,
  const double & pathDensity, vector<WayPoint> & extractedPath, const double & SmoothDataWeight,
...
//		cout << "Aler Alert !!! fast: " << close_index << ", slow: " << i_slow  << endl;
  ...
  double d_limit = 0;
  if (close_index >= 2) {close_index -= 2;} else {close_index = 0;}

  for (unsigned int i = close_index; i < originalPath.size(); i++) {
    extractedPath.push_back(originalPath.at(i));

    if (i > 0) {
      d_limit += hypot(originalPath.at(i).pos.y - originalPath.at(i - 1).pos.y, originalPath.at(
            i).pos.x - originalPath.at(i - 1).pos.x);
    }
  ...
  if (extractedPath.size() < 2) {
    cout << endl << "### Planner Z . Extracted Rollout Path is too Small, Size = " <<
  ...
  const double & pathDensity, vector<WayPoint> & extractedPath)
{
  ...
  for (int i = close_index; i >= 0; i--) {
    ...
    if (i < originalPath.size()) {
      d += hypot(originalPath.at(i).pos.y - originalPath.at(i + 1).pos.y, originalPath.at(
            i).pos.x - originalPath.at(i + 1).pos.x);
    }
    if (d > 10) {
      break;
...
void PlanningHelpers::CalculateRollInTrajectories(
  const WayPoint & carPos, const double & speed, const vector<WayPoint> & originalCenter,
  int & start_index,
...
//	}
  ...
  for (unsigned int j = smoothing_start_index; j < end_index; j++) {
    ...
    for (unsigned int i = 0; i < rollOutNumber + 1; i++) {
      ...
      p.pos.x = originalCenter.at(j).pos.x - initial_roll_in_distance * cos(p.pos.a + M_PI_2) - d *
        cos(p.pos.a + M_PI_2);
      p.pos.y = originalCenter.at(j).pos.y - initial_roll_in_distance * sin(p.pos.a + M_PI_2) - d *
        sin(p.pos.a + M_PI_2);
      if (i != centralTrajectoryIndex) {
        p.v = original_speed * LANE_CHANGE_SPEED_FACTOR;
      } else {
  ...
  for (unsigned int i = 0; i < rollOutNumber + 1; i++) {
    rollInPaths.at(i).insert(rollInPaths.at(i).begin(), execluded_from_smoothing.at(
        i).begin(), execluded_from_smoothing.at(i).end());
  }
...
void PlanningHelpers::SmoothSpeedProfiles(
  vector<WayPoint> & path_in, double weight_data,
  double weight_smooth, double tolerance)
{
...
void PlanningHelpers::SmoothCurvatureProfiles(
  vector<WayPoint> & path_in, double weight_data,
  double weight_smooth, double tolerance)
{
  if (path_in.size() <= 1) {
...
void PlanningHelpers::SmoothWayPointsDirections(
  vector<WayPoint> & path_in, double weight_data,
  double weight_smooth, double tolerance)
{
  ...
  while (change >= tolerance) {
    ...
    for (int i = 1; i < size - 1; i++) {
      ...
      newpath[i].pos.a += weight_smooth *
        (newpath[i - 1].pos.a + newpath[i + 1].pos.a - (2.0 * newpath[i].pos.a));
  ...
  *wp = *pStart;
  ...
  while (nextLeafToTrace.size() > 0) {
    ...
    if (distance_to_goal <= 0.1 && angle_to_goal < M_PI_4) {
      ...
      if (pH->pLeft &&
        !CheckLaneExits(all_cells_to_delete,
        pH->pLeft->pLane) &&
        !CheckNodeExits(all_cells_to_delete,
        pH->pLeft) && bEnableLaneChange && before_change_distance > LANE_CHANGE_MIN_DISTANCE)
      {
        wp = new WayPoint();
        *wp = *pH->pLeft;
      ...
      if (pH->pRight &&
        ...
        pH->pRight->pLane) &&
        !CheckNodeExits(all_cells_to_delete,
        pH->pRight) && bEnableLaneChange && before_change_distance > LANE_CHANGE_MIN_DISTANCE)
      {
        wp = new WayPoint();
        *wp = *pH->pRight;
        double d = hypot(wp->pos.y - pH->pos.y, wp->pos.x - pH->pos.x);
      ...
      for (unsigned int i = 0; i < pH->pFronts.size(); i++) {
        if (CheckLaneIdExits(globalPath,
          pH->pLane) && pH->pFronts.at(i) &&
...
int PlanningHelpers::PredictiveDP(
  WayPoint * pStart, const double & DistanceLimit,
  vector<WayPoint *> & all_cells_to_delete, vector<WayPoint *> & end_waypoints)
{
  if (!pStart) {return 0;}
...
WayPoint * PlanningHelpers::CreateLaneHeadCell(
  Lane * pLane, WayPoint * pLeft, WayPoint * pRight,
  WayPoint * pBack)
{
...
double PlanningHelpers::GetLanePoints(
  Lane * l, const WayPoint & prevWayPointIndex,
...
WayPoint * PlanningHelpers::GetMinCostCell(
  const vector<WayPoint *> & cells,
  ...
  std::vector<std::vector<WayPoint>> & allPaths)
{
  ...
  for (unsigned int i = 1; i < singlePath.size(); i++) {
      ...
      singlePath.at(i).pFronts.size() > 0)
    {
      ...
      WayPoint * pLaneCell = PlanningHelpers::BuildPlanningSearchTreeStraight(pStart,
          BACKUP_STRAIGHT_PLAN_DISTANCE,
          local_cell_to_delete);
      if (pLaneCell) {
        vector<WayPoint> straight_path;
        vector<vector<WayPoint>> tempCurrentForwardPathss;
        vector<int> globalPathIds;
        PlanningHelpers::TraversePathTreeBackwards(pLaneCell, pStart, globalPathIds, straight_path,
    ...
    if (bStartSkip) {
      d += hypot(singlePath.at(i).pos.y - singlePath.at(i - 1).pos.y, singlePath.at(
            i).pos.x - singlePath.at(i - 1).pos.x);
      if (d > skip_distance) {
        d = 0;
        bStartSkip = false;
      }
  ...
  vector<WayPoint> & localPath, std::vector<std::vector<WayPoint>> & localPaths)
{
  if (pHead != NULL && pHead->id != pStartWP->id) {
    if (pHead->pBacks.size() > 0) {
      ...
      TraversePathTreeBackwards(GetMinCostCell(pHead->pBacks,
        globalPathIds), pStartWP, globalPathIds, localPath,
        localPaths);
...
void PlanningHelpers::CalcContourPointsForDetectedObjects(
  ...
  vector<DetectedObject> & obj_list,
...
double PlanningHelpers::GetVelocityAhead(
  const std::vector<WayPoint> & path,
  ...
  const double & reasonable_brake_distance)
{
  ...
  while (local_i < path.size() - 1 && d < reasonable_brake_distance) {
    local_i++;
    d += hypot(path.at(local_i).pos.y - path.at(local_i - 1).pos.y, path.at(local_i).pos.x - path.at(
          local_i - 1).pos.x);
...
void PlanningHelpers::WritePathToFile(const string & fileName, const vector<WayPoint> & path)
{
  ...
  str_header << "laneID" << "," << "wpID" << "," "x" << "," << "y" << "," << "a" << "," << "cost" <<
      "," << "Speed" << ",";
  vector<string> dataList;
  for (unsigned int i = 0; i < path.size(); i++) {
    ostringstream strwp;
    strwp << path.at(i).laneId << "," << path.at(i).id << "," << path.at(i).pos.x << "," << path.at(
      i).pos.y <<
  ...
  const double & seachDistance)
{
  ...
  for (unsigned int i = info.iFront; i < path.size() - 2; i++) {
    ...
    total_d += hypot(path.at(i + 1).pos.y - path.at(i).pos.y, path.at(i + 1).pos.x - path.at(
          i).pos.x);
...
PlannerHNS::WayPoint PlanningHelpers::GetRealCenter(
