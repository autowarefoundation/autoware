...
void BehaviorPrediction::CalculateCollisionTimes(const double & minSpeed)
{
  for (unsigned int i = 0; i < m_ParticleInfo_II.size(); i++) {
    for (unsigned int j = 0; j < m_ParticleInfo_II.at(i)->obj.predTrajectories.size(); j++) {
      PlannerHNS::PlanningHelpers::PredictConstantTimeCostForTrajectory(m_ParticleInfo_II.at(
          i)->obj.predTrajectories.at(j), m_ParticleInfo_II.at(
          i)->obj.center, minSpeed, m_PredictionDistance);
...
void BehaviorPrediction::CalPredictionTimeForObject(ObjParticles * pCarPart)
{
  if (pCarPart->obj.center.v > 0) {
    pCarPart->m_PredictionTime = (MIN_PREDICTION_TIME + 1.5 * pCarPart->obj.center.v) /
...
void BehaviorPrediction::PredictCurrentTrajectory(RoadNetwork & map, ObjParticles * pCarPart)
{
  ...
  if (pCarPart->obj.bDirection && pCarPart->obj.bVelocity) {
    ...
    pCarPart->obj.pClosestWaypoints = MappingHelpers::GetClosestWaypointsListFromMap(fake_pose, map,
        m_MaxLaneDetectionDistance,
        pCarPart->obj.bDirection);
    planner.PredictTrajectoriesUsingDP(fake_pose, pCarPart->obj.pClosestWaypoints,
    ...
    pCarPart->obj.pClosestWaypoints = MappingHelpers::GetClosestWaypointsListFromMap(
      pCarPart->obj.center, map, m_MaxLaneDetectionDistance, pCarPart->obj.bDirection);
    ...
    planner.PredictTrajectoriesUsingDP(pCarPart->obj.center, pCarPart->obj.pClosestWaypoints,
      m_PredictionDistance, pCarPart->obj.predTrajectories,
      m_bGenerateBranches, pCarPart->obj.bDirection);
  }
...
double BehaviorPrediction::CalcIndicatorWeight(
  PlannerHNS::LIGHT_INDICATOR p_ind,
  PlannerHNS::LIGHT_INDICATOR obj_ind)
{
  if ((obj_ind == PlannerHNS::INDICATOR_LEFT || obj_ind == PlannerHNS::INDICATOR_RIGHT ||
    obj_ind == PlannerHNS::INDICATOR_NONE) && p_ind == obj_ind)
  {
    return 0.99;
  } else {
    return 0.01;
...
void BehaviorPrediction::CalOnePartWeight(ObjParticles * pParts, Particle & p)
{
  ...
  p.w_raw = p.pose_w * POSE_FACTOR + p.dir_w * DIRECTION_FACTOR + p.vel_w * VELOCITY_FACTOR +
    p.ind_w * INDICATOR_FACTOR + p.acl_w * ACCELERATE_FACTOR;
...
void BehaviorPrediction::NormalizeOnePartWeight(ObjParticles * pParts, Particle & p)
{
  ...
  p.w = p.pose_w * POSE_FACTOR + p.dir_w * DIRECTION_FACTOR + p.vel_w * VELOCITY_FACTOR + p.ind_w *
    INDICATOR_FACTOR + p.acl_w * ACCELERATE_FACTOR;
...
void BehaviorPrediction::CalculateWeights(ObjParticles * pParts)
{
  ...
  if ((pParts->max_w_raw == 0 || fabs(pParts->max_w_raw - pParts->min_w_raw) < 0.1 ||
    pParts->min_w_raw > 0.5) && pParts->m_TrajectoryTracker.size() > 1)
  {
    m_bCanDecide = false;
  } else {
    m_bCanDecide = true;
...
//		std::cout << "Brake Eeeeee Eeeeeeee: " << std::endl;
  ...
  for (int i = 0; i < pParts->m_AllParticles.size(); i++) {
    ...
    double d = hypot(pParts->obj.center.pos.y - pParts->m_AllParticles.at(
          i)->pose.pos.y, pParts->obj.center.pos.x - pParts->m_AllParticles.at(i)->pose.pos.x);

    if (pParts->m_AllParticles.at(i)->w < critical_val || d > m_PredictionDistance) {
      pParts->m_AllParticles.at(i)->pTraj->DeleteParticle(*(pParts->m_AllParticles.at(
          i)), pParts->m_AllParticles.at(i)->original_index);
...
void BehaviorPrediction::FindBest(ObjParticles * pParts)
{
  ...
  if (m_bDebugOut) {
    std::cout << "Behavior Prob ------------------------------------------------ : " <<
        pParts->m_TrajectoryTracker.size() << std::endl;
    std::cout << "Raw  Weights: Max: " << pParts->max_w_raw << ", Min: " << pParts->min_w_raw <<
        std::endl;
  ...
  if (m_bCanDecide && pParts->best_beh_track != nullptr) {
    ...
    if (m_bDebugOut) {
      std::cout << "Trajectory (" << pParts->i_best_track << "), P: " <<
          pParts->best_beh_track->best_p << " , Beh (" << pParts->best_beh_track->best_beh <<
          ", " << str_beh << ")" << std::endl;
    }
    ...
    for (unsigned int t = 0; t < pParts->m_TrajectoryTracker.size(); t++) {
      if (pParts->m_TrajectoryTracker.at(t)->nAliveForward > 0) {
        std::cout << t << ", Forward Particles:" <<
            pParts->m_TrajectoryTracker.at(t)->nAliveForward << std::endl;
      }
      if (pParts->m_TrajectoryTracker.at(t)->nAliveStop > 0) {
        std::cout << t << ", Stoping Particles:" << pParts->m_TrajectoryTracker.at(t)->nAliveStop <<
            std::endl;
      }
    }
...
//	}
  ...
  for (unsigned int t = 0; t < pParts->m_TrajectoryTracker.size(); t++) {
    RelativeInfo info;
    PlanningHelpers::GetRelativeInfo(pParts->m_TrajectoryTracker.at(
        t)->trajectory, pParts->obj.center, info);
    unsigned int point_index = 0;
    p.pose = PlanningHelpers::GetFollowPointOnTrajectory(pParts->m_TrajectoryTracker.at(
          t)->trajectory, info, PREDICTION_DISTANCE_PERCENTAGE * m_PredictionDistance, point_index);

    if (pParts->m_TrajectoryTracker.at(t)->beh == PlannerHNS::BEH_FORWARD_STATE &&
...
void BehaviorPrediction::MoveParticles(ObjParticles * pParts)
{
  ...
  for (unsigned int t = 0; t < pParts->m_TrajectoryTracker.size(); t++) {
    PlanningHelpers::GenerateRecommendedSpeed(pParts->m_TrajectoryTracker.at(
        t)->trajectory, carInfo.max_speed_forward, 1.0);
  }
