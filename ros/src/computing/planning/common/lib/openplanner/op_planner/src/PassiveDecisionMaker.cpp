...
double PassiveDecisionMaker::GetVelocity(
  ...
  const CAR_BASIC_INFO & carInfo, const RelativeInfo & info)
{
  ...
  int prev_index = 0;
  double velocity = PlannerHNS::PlanningHelpers::GetVelocityAhead(path, info, prev_index,
...
bool PassiveDecisionMaker::CheckForStopLine(
  PlannerHNS::WayPoint & currPose,
  const std::vector<WayPoint> & path,
  const CAR_BASIC_INFO & carInfo)
{
  ...
  distanceToClosestStopLine = PlanningHelpers::GetDistanceToClosestStopLineAndCheck(path, currPose,
      0, stopLineID,
      stopSignID,
      trafficLightID)
...
PlannerHNS::BehaviorState PassiveDecisionMaker::MoveStep(
  const double & dt,
  PlannerHNS::WayPoint & currPose,
  const std::vector<WayPoint> & path,
  const CAR_BASIC_INFO & carInfo)
