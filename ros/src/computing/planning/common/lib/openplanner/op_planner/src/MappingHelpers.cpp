...
void MappingHelpers::ConstructRoadNetworkFromROSMessage(
  const std::vector<op_utility_ns::AisanLanesFileReader::AisanLane> & lanes_data,
...
//					prev_FLID = 0;
    ...
    WayPoint wp;
    bool bFound = GetWayPoint(curr_lane_point.LnID, lane_obj.id, curr_lane_point.RefVel,
        curr_lane_point.DID,
...
//		}
  ...
  for (unsigned int l = 0; l < roadLanes.size(); l++) {
    for (unsigned int fp = 0; fp < roadLanes.at(l).fromIds.size(); fp++) {
      roadLanes.at(l).fromIds.at(fp) = GetLaneIdByWaypointId(roadLanes.at(l).fromIds.at(
            fp), roadLanes);
    }
...
void MappingHelpers::ConstructRoadNetworkFromDataFiles(const std::string vectoMapPath,
  RoadNetwork & map, const bool & bZeroOrigin)
{
  /**
  ...
  if (nodes_data.size() > 0 && bSpecialMap == 0) {
    ConstructRoadNetworkFromROSMessageV2(lanes_data, points_data, dt_data, intersection_data,
      area_data,
    ...
    ConstructRoadNetworkFromROSMessage(lanes_data, points_data, dt_data, intersection_data,
      area_data,
      line_data, stop_line_data, signal_data, vector_data, curb_data, roadedge_data,
...
//					double B = deg + min + sec;
          ...
          wp.pos = GPSPoint(points.at(p).Ly + origin.x, points.at(p).Bx + origin.y, points.at(
                p).H + origin.z, dtpoints.at(dtp).Dir);
...
void MappingHelpers::LoadKML(const std::string & kmlFile, RoadNetwork & map)
{
    ...
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      ...
      for (unsigned int isl = 0; isl < map.stopLines.size(); isl++) {
        if (map.stopLines.at(isl).laneId == map.roadSegments.at(rs).Lanes.at(i).id) {
          ...
          WayPoint wp((map.stopLines.at(isl).points.at(0).x + map.stopLines.at(isl).points.at(
              1).x) / 2.0, (map.stopLines.at(isl).points.at(0).y + map.stopLines.at(isl).points.at(
              1).y) / 2.0, (map.stopLines.at(isl).points.at(0).z + map.stopLines.at(isl).points.at(
              1).z) / 2.0, (map.stopLines.at(isl).points.at(0).a + map.stopLines.at(isl).points.at(
              1).a) / 2.0);
          map.roadSegments.at(rs).Lanes.at(i).points.at(PlanningHelpers::GetClosestNextPointIndexFast(
              map.roadSegments.at(
                rs).Lanes.at(i).points, wp)).stopLineID = map.stopLines.at(isl).id;
        }
      }
    }
  }
  ...
  cout << "Map loaded from kml file with (" << laneLinksList.size() << ") lanes, First Point ( " <<
      GetFirstWaypoint(map).pos.ToString() << ")" << endl;
...
vector<WayPoint *> MappingHelpers::GetClosestWaypointsListFromMap(const WayPoint & pos,
  RoadNetwork & map,
...
std::vector<Lane *> MappingHelpers::GetClosestLanesFast(const WayPoint & center, RoadNetwork & map,
  const double & distance)
{
  ...
  for (unsigned int j = 0; j < map.roadSegments.size(); j++) {
    for (unsigned int k = 0; k < map.roadSegments.at(j).Lanes.size(); k++) {
      ...
      double d = hypot(pL->points.at(index).pos.y - center.pos.y, pL->points.at(
            index).pos.x - center.pos.x);
      if (d <= distance) {
...
Lane * MappingHelpers::GetClosestLaneFromMap(const WayPoint & pos, RoadNetwork & map,
  const double & distance, const bool bDirectionBased)
{
...
vector<Lane *> MappingHelpers::GetClosestLanesListFromMap(const WayPoint & pos, RoadNetwork & map,
  const double & distance,
  const bool bDirectionBased)
{
  vector<pair<double, Lane *>> laneLinksList;
...
Lane * MappingHelpers::GetClosestLaneFromMapDirectionBased(const WayPoint & pos, RoadNetwork & map,
  const double & distance)
{
  ...
  for (unsigned int j = 0; j < map.roadSegments.size(); j++) {
    for (unsigned int k = 0; k < map.roadSegments.at(j).Lanes.size(); k++) {
      ...
      if (min_d < distance) {
        laneLinksList.push_back(make_pair(min_d,
          &map.roadSegments.at(j).Lanes.at(k).points.at(min_i)));
      }
    }
  }
...
std::vector<Lane *> MappingHelpers::GetClosestMultipleLanesFromMap(const WayPoint & pos,
  RoadNetwork & map,
  const double & distance)
{
  vector<Lane *> lanesList;
...
WayPoint * MappingHelpers::GetLastWaypoint(RoadNetwork & map)
{
  if (map.roadSegments.size() > 0 &&
    ...
    if (lanes->at(lanes->size() - 1).points.size() > 0) {
      return &lanes->at(lanes->size() - 1).points.at(lanes->at(lanes->size() - 1).points.size() -
               1);
    }
  }
...
void MappingHelpers::GetUniqueNextLanes(const Lane * l, const vector<Lane *> & traversed_lanes,
  ...
  if (pV) {
    ...
    if (gps_points.size() == add_info_list.size()) {
      for (unsigned int i = 0; i < gps_points.size(); i++) {
        ...
        gps_points.at(i).actionCost.push_back(GetActionPairFromPrefix(add_info_list.at(i), "AC",
          "From"));
  ...
  if (pV) {
    ...
    if (gps_points.size() == add_info_list.size()) {
      for (unsigned int i = 0; i < gps_points.size(); i++) {
        ...
        gps_points.at(i).pos.a = gps_points.at(i).pos.dir = GetDoubleFromPrefix(add_info_list.at(
              i), "Dir", "").at(0);
//				if(currLaneID == 11115)
//				{
//
...
vector<int> MappingHelpers::GetIDsFromPrefix(const string & str, const string & prefix,
...
vector<double> MappingHelpers::GetDoubleFromPrefix(const string & str, const string & prefix,
  const string & postfix)
{
  int index1 = str.find(prefix) + prefix.size();
  int index2 = str.find(postfix, index1);
...
pair<ACTION_TYPE, double> MappingHelpers::GetActionPairFromPrefix(const string & str,
  const string & prefix,
  const string & postfix)
{
  int index1 = str.find(prefix) + prefix.size();
    ...
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      ...
      for (unsigned int rs_2 = 0; rs_2 < map.roadSegments.size(); rs_2++) {
        for (unsigned int i2 = 0; i2 < map.roadSegments.at(rs_2).Lanes.size(); i2++) {
          ...
          int iCenter2 = PlanningHelpers::GetClosestNextPointIndexFast(map.roadSegments.at(
                rs_2).Lanes.at(i2).points, wp_1);
              ...
              for (unsigned int i_internal = 0; i_internal < pL->points.size(); i_internal++) {
                if (i_internal < map.roadSegments.at(rs_2).Lanes.at(i2).points.size()) {
                  ...
                  pL->points.at(i_internal).pRight =
                    &map.roadSegments.at(rs_2).Lanes.at(i2).points.at(i_internal);
        ...
        if (signal_data.at(is).VID == vector_data.at(iv).VID) {
          for (unsigned int ip = 0; ip < points_data.size(); ip++) {
            if (vector_data.at(iv).PID == points_data.at(ip).PID) {
              tl.pos = GPSPoint(points_data.at(ip).Ly + origin.x, points_data.at(
                    ip).Bx + origin.y, points_data.at(ip).H + origin.z, vector_data.at(
                    iv).Hang * DEG2RAD);
              break;
            }
          }
        ...
        for (unsigned int ip = 0; ip < points_data.size(); ip++) {
          if (points_data.at(ip).PID == s_id || points_data.at(ip).PID == e_id) {
            sl.points.push_back(GPSPoint(points_data.at(ip).Ly + origin.x,
              points_data.at(ip).Bx + origin.y, points_data.at(ip).H + origin.z, 0));
          }
        }
      }
    }
          ...
          if (line_data.at(il).LID >= s_id && line_data.at(il).LID <= e_id) {
            for (unsigned int ip = 0; ip < points_data.size(); ip++) {
              if (points_data.at(ip).PID == line_data.at(il).BPID) {
                GPSPoint p(points_data.at(ip).Ly + origin.x, points_data.at(
                    ip).Bx + origin.y, points_data.at(ip).H + origin.z, 0);
    ...
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      ...
      for (unsigned int p = 0; p < pLane->points.size(); p++) {
        ...
        if (p + 1 == pLane->points.size()) {                    // Last Point in Lane
          ...
          if (pWP->toIds.size() > 1) {
            cout << "Error Error Erro ! Lane: " << pWP->laneId << ", Point: " <<
                pWP->originalMapID << endl;
          } else {
        ...
        if (map.roadSegments.at(rs).Lanes.at(i).id == data_conn.LID) {
          ...
          for (unsigned int isl = 0; isl < map.stopLines.size(); isl++) {
            if (map.stopLines.at(isl).id == data_conn.SLID) {
              ...
              map.roadSegments.at(rs).Lanes.at(i).stopLines.push_back(map.stopLines.at(isl));
              WayPoint wp((map.stopLines.at(isl).points.at(0).x + map.stopLines.at(isl).points.at(
                  1).x) / 2.0, (map.stopLines.at(isl).points.at(0).y + map.stopLines.at(
                  isl).points.at(1).y) / 2.0, (map.stopLines.at(isl).points.at(
                  0).z + map.stopLines.at(isl).points.at(1).z) / 2.0, (map.stopLines.at(
                  isl).points.at(0).a + map.stopLines.at(isl).points.at(1).a) / 2.0);
              map.roadSegments.at(rs).Lanes.at(i).points.at(PlanningHelpers::GetClosestNextPointIndexFast(
                  map.roadSegments.at(
                    rs).Lanes.at(i).points, wp)).stopLineID = map.stopLines.at(isl).id;
            }
...
void MappingHelpers::UpdateMapWithOccupancyGrid(OccupancyToGridMap & map_info,
  const std::vector<int> & data, RoadNetwork & map,
...
bool MappingHelpers::GetPointFromDataList(op_utility_ns::AisanPointsFileReader * pPointsData,
...
bool MappingHelpers::IsStartLanePoint(op_utility_ns::AisanLanesFileReader * pLaneData,
  op_utility_ns::AisanLanesFileReader::AisanLane * pL)
{
...
bool MappingHelpers::IsEndLanePoint(op_utility_ns::AisanLanesFileReader * pLaneData,
  op_utility_ns::AisanLanesFileReader::AisanLane * pL)
{
...
void MappingHelpers::ConnectLanes(op_utility_ns::AisanLanesFileReader * pLaneData,
...
void MappingHelpers::CreateLanes(
  op_utility_ns::AisanLanesFileReader * pLaneData,
  op_utility_ns::AisanPointsFileReader * pPointsData,
  op_utility_ns::AisanNodesFileReader * pNodesData,
  ...
  for (unsigned int il = 0; il < lanes.size(); il++) {
    for (int ip = 1; ip < lanes.at(il).points.size(); ip++) {
      ...
      if (d == 0) {
        ...
        if (DEBUG_MAP_PARSING) {
          cout << "Fixed Redundant Points for Lane:" << lanes.at(il).id << ", Current: " << ip <<
    ...
    if (lanes.at(il).fromIds.size() == 0) {
      ...
      for (int l = 0; l < sp_lanes.size(); l++) {
        if (pFL->id != sp_lanes.at(l).id) {
          ...
          PlanningHelpers::GetRelativeInfoLimited(sp_lanes.at(l).points, pFL->points.at(0), info,
            0);
          double back_distance = hypot(lastofother.pos.y - pFL->points.at(
                0).pos.y, lastofother.pos.x - pFL->points.at(0).pos.x);
          ...
          if (fabs(info.perp_distance) < 2 && fabs(info.perp_distance) < closest_d &&
            info.bBefore == false && bCloseFromBack) {
      ...
      if (pL != nullptr && pFL != nullptr) {
        if (closest_info.iFront == pL->points.size() - 1) {
          ...
          if (DEBUG_MAP_PARSING) {
            cout << "Closest Next Lane For: " << pFL->id << " , Is:" << pL->id << ", Distance=" <<
                fabs(closest_info.perp_distance) << ", Size: " << pL->points.size() <<
                ", back_index: " << closest_info.iBack << ", front_index: " <<
                closest_info.iFront << ", Direct: " << closest_info.angle_diff << endl;
          ...
          front_half.points.insert(front_half.points.begin(),
            pL->points.begin() + closest_info.iFront, pL->points.end());
          ...
          if (front_half.points.size() > 1 && back_half.points.size() > 1) {
            if (DEBUG_MAP_PARSING) {
              cout << "Split this one Nicely! first_half_size: " << front_half.points.size() <<
    ...
    if (lanes.at(il).toIds.size() == 0) {
      ...
      for (int l = 0; l < sp_lanes.size(); l++) {
        if (pBL->id != sp_lanes.at(l).id) {
          ...
          if (fabs(info.perp_distance) < 2 && fabs(info.perp_distance) < closest_d &&
            info.bAfter == false && bCloseFromFront) {
            closest_d = fabs(info.perp_distance);
            pL = &sp_lanes.at(l);
            closest_info = info;
      ...
      if (pL != nullptr && pBL != nullptr) {
        if (closest_info.iFront == 0) {
          ...
          pL->points.at(closest_info.iFront).fromIds.push_back(pBL->points.at(
              pBL->points.size() - 1).id);

          pBL->points.at(pBL->points.size() - 1).toIds.push_back(pL->points.at(
              closest_info.iFront).id);
          pBL->toIds.push_back(pL->id);

          bAtleastOneChange = true;
          if (DEBUG_MAP_PARSING) {
            cout << "Closest Back Lane For: " << pBL->id << " , Is:" << pL->id << ", Distance=" <<
                fabs(closest_info.perp_distance) << ", Size: " << pL->points.size() <<
                ", back_index: " << closest_info.iBack << ", front_index: " <<
                closest_info.iFront << ", Direct: " << closest_info.angle_diff << endl;
            cout << "Don't Split , Perfect !" << endl;
  ...
  const GPSPoint & origin, RoadNetwork & map)
{
  for (unsigned int ist = 0; ist < stop_line_data.size(); ist++) {
    ...
    if (pLine != nullptr) {
      ...
      if (pEnd != nullptr) {
        sl.points.push_back(GPSPoint(pEnd->Ly + origin.x, pEnd->Bx + origin.y, pEnd->H + origin.z,
          0));
      }
    }
