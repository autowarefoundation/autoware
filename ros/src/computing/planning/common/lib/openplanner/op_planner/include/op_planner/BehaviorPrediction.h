  ...
  double CalcMatchingPercentage(const std::vector<PlannerHNS::WayPoint> & _path)
  {
    ...
    for (unsigned int i = 0; i < _path.size(); i++) {
      if (i < trajectory.size()) {
        rms_val += hypot(_path.at(i).pos.y - trajectory.at(i).pos.y, _path.at(
              i).pos.y - trajectory.at(i).pos.y);
      }
    }
...
public:
  ...
  void DoOneStep(
    const std::vector<DetectedObject> & obj_list, const WayPoint & currPose,
    const double & minSpeed, const double & maxDeceleration, RoadNetwork & map);
...
protected:
  ...
  void ExtractTrajectoriesFromMap(
    const std::vector<DetectedObject> & obj_list, RoadNetwork & map,
