...
void SimuDecisionMaker::GenerateLocalRollOuts()
{
  ...
  for (unsigned int i = 0; i < _roll_outs.size(); i++) {
    for (unsigned int j = 0; j < _roll_outs.at(i).size(); j++) {
      PlannerHNS::PlanningHelpers::PredictConstantTimeCostForTrajectory(_roll_outs.at(i).at(
          j), state, m_params.minSpeed, m_params.microPlanDistance);
...
PlannerHNS::VehicleState SimuDecisionMaker::LocalizeStep(
  const double & dt,
  ...
  const bool & bEmergencyStop)
{
  ...
  PlannerHNS::TrajectoryCost tc = m_TrajectoryCostsCalculator.DoOneStepStatic(m_RollOuts, m_TotalPath.at(
        m_iCurrentTotalPathId), state, m_params, m_CarInfo, vehicleState, objects);
