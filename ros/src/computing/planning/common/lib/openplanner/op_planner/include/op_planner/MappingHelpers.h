...
public:
  ...
  static void ConstructRoadNetworkFromDataFiles(
    const std::string vectoMapPath, RoadNetwork & map,
    const bool & bZeroOrigin = false);

  static void UpdateMapWithOccupancyGrid(
    OccupancyToGridMap & map_info,
  ...
  static Lane * GetClosestLaneFromMap(
    const WayPoint & pos, RoadNetwork & map,
    const double & distance = 5.0,
    const bool bDirectionBased = true);
  static std::vector<Lane *> GetClosestLanesListFromMap(
    const WayPoint & pos, RoadNetwork & map,
    const double & distance = 2.0,
    const bool bDirectionBased = true);
  static Lane * GetClosestLaneFromMapDirectionBased(
    const WayPoint & pos, RoadNetwork & map,
    const double & distance = 5.0);
  static std::vector<Lane *> GetClosestMultipleLanesFromMap(
    const WayPoint & pos, RoadNetwork & map,
    const double & distance = 5.0);
  static WayPoint * GetClosestWaypointFromMap(
    const WayPoint & pos, RoadNetwork & map,
    const bool bDirectionBased = true);
  static std::vector<Lane *> GetClosestLanesFast(
    const WayPoint & pos, RoadNetwork & map,
    const double & distance = 10.0);

  static std::vector<WayPoint *> GetClosestWaypointsListFromMap(
  ...
  static void GetUniqueNextLanes(
    const Lane * l, const std::vector<Lane *> & traversed_lanes,
  ...
  static std::vector<int> GetIDsFromPrefix(
    const std::string & str, const std::string & prefix,
    const std::string & postfix);
  static std::vector<double> GetDoubleFromPrefix(
    const std::string & str,
    const std::string & prefix,
    const std::string & postfix);
  static std::pair<ACTION_TYPE, double> GetActionPairFromPrefix(
    const std::string & str,
    const std::string & prefix,
    const std::string & postfix);
  ...
  static bool GetPointFromDataList(
    op_utility_ns::AisanPointsFileReader * pPointsData,
    const int & pid, WayPoint & out_wp);

  static int GetBeginPointIdFromLaneNo(
    op_utility_ns::AisanLanesFileReader * pLaneData,
    op_utility_ns::AisanPointsFileReader * pPointsData,
    op_utility_ns::AisanNodesFileReader * pNodesData, const int & LnID);
  static int GetEndPointIdFromLaneNo(
    op_utility_ns::AisanLanesFileReader * pLaneData,
    op_utility_ns::AisanPointsFileReader * pPointsData,
    op_utility_ns::AisanNodesFileReader * pNodesData, const int & LnID);

  static bool IsStartLanePoint(
    op_utility_ns::AisanLanesFileReader * pLaneData,
    op_utility_ns::AisanLanesFileReader::AisanLane * pL);
  static bool IsEndLanePoint(
    op_utility_ns::AisanLanesFileReader * pLaneData,
    op_utility_ns::AisanLanesFileReader::AisanLane * pL);
