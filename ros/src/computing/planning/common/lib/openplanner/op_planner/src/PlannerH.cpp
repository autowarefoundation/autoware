...
void PlannerH::GenerateRunoffTrajectory(
  const std::vector<std::vector<WayPoint>> & referencePaths, const WayPoint & carPos,
  const bool & bEnableLaneChange, const double & speed, const double & microPlanDistance,
  const double & maxSpeed, const double & minSpeed, const double & carTipMargin,
  ...
  std::vector<WayPoint> & sampledPoints_debug)
{
  ...
  for (unsigned int i = 0; i < referencePaths.size(); i++) {
    ...
    if (referencePaths.at(i).size() > 0) {
      PlanningHelpers::CalculateRollInTrajectories(carPos, speed, referencePaths.at(
          i), s_index, e_index, e_distances,
...
double PlannerH::PlanUsingDPRandom(
  const WayPoint & start,
  const double & maxPlanningDistance,
  RoadNetwork & map,
  std::vector<std::vector<WayPoint>> & paths)
{
  PlannerHNS::WayPoint * pStart = PlannerHNS::MappingHelpers::GetClosestBackWaypointFromMap(start,
      map);

  if (!pStart) {
    GPSPoint sp = start.pos;
    cout << endl << "Error: PlannerH -> Can't Find Global Waypoint Nodes in the Map for Start (" <<
        sp.ToString() << ")" << endl;
    return 0;
  }
  ...
  pLaneCell = PlanningHelpers::BuildPlanningSearchTreeStraight(pStart, maxPlanningDistance,
      local_cell_to_delete);

  if (!pLaneCell) {
    cout << endl << "PlannerH -> Plan (B) Failed, Sorry we Don't have plan (C) This is the END." <<
        endl;
    return 0;
  }
  ...
  PlanningHelpers::TraversePathTreeBackwards(pLaneCell, pStart, globalPath, path,
    tempCurrentForwardPathss);
  cout << endl << "Info: PlannerH -> Plan (B) Path With Size (" << (int)path.size() <<
  ...
  std::vector<std::vector<WayPoint>> & paths, vector<WayPoint *> * all_cell_to_delete)
{
  ...
  if (!pStart->pLane || !pGoal->pLane) {
    cout << endl << "Error: PlannerH -> Null Lane, Start (" << pStart->pLane << ") and Goal (" <<
        pGoal->pLane << ")" << endl;
    return 0;
  }
  ...
  if (all_cell_to_delete) {
    pLaneCell = PlanningHelpers::BuildPlanningSearchTreeV2(pStart, *pGoal, globalPath,
        maxPlanningDistance, bEnableLaneChange,
        *all_cell_to_delete);
  } else {
    pLaneCell = PlanningHelpers::BuildPlanningSearchTreeV2(pStart, *pGoal, globalPath,
        maxPlanningDistance, bEnableLaneChange,
  ...
  if (!pLaneCell) {
    ...
    if (all_cell_to_delete) {
      pLaneCell = PlanningHelpers::BuildPlanningSearchTreeStraight(pStart,
    ...
    if (!pLaneCell) {
      bPlan = 'Z';
      cout << endl <<
          "PlannerH -> Plan (B) Failed, Sorry we Don't have plan (C) This is the END." << endl;
      return 0;
    }
  }
  ...
  cout << endl << "Info: PlannerH -> Plan (" << bPlan << ") Path With Size (" << (int)path.size() <<
      "), MultiPaths No(" << paths.size() << ") Extraction Time : " << endl;
...
double PlannerH::PredictPlanUsingDP(
  PlannerHNS::Lane * l, const WayPoint & start,
  const double & maxPlanningDistance,
  std::vector<std::vector<WayPoint>> & paths)
{
  if (!l) {
  ...
  if (distance2points(closest_p.pos, carPos.pos) > 8) {
    cout << endl << "Err: PredictiveDP PlannerH -> Distance to Lane is: " << distance2points(
      closest_p.pos, carPos.pos) <<
  ...
  int nPaths = PlanningHelpers::PredictiveDP(pStartWP, maxPlanningDistance, all_cell_to_delete,
      pLaneCells);
  ...
  for (unsigned int i = 0; i < pLaneCells.size(); i++) {
    ...
    PlanningHelpers::TraversePathTreeBackwards(pLaneCells.at(
        i), pStartWP, globalPath, path, tempCurrentForwardPathss);
    if (path.size() > 0) {
...
double PlannerH::PredictTrajectoriesUsingDP(
  const WayPoint & startPose,
  std::vector<WayPoint *> closestWPs,
  const double & maxPlanningDistance,
  std::vector<std::vector<WayPoint>> & paths,
  const bool & bFindBranches, const bool bDirectionBased,
  const bool pathDensity)
{
  vector<vector<WayPoint>> tempCurrentForwardPathss;
  vector<WayPoint *> all_cell_to_delete;
  vector<int> globalPath;
  ...
  for (unsigned int j = 0; j < closestWPs.size(); j++) {
    pLaneCells.clear();
    int nPaths = PlanningHelpers::PredictiveIgnorIdsDP(closestWPs.at(
          j), maxPlanningDistance, all_cell_to_delete, pLaneCells, unique_lanes);
    for (unsigned int i = 0; i < pLaneCells.size(); i++) {
      path.clear();
      PlanningHelpers::TraversePathTreeBackwards(pLaneCells.at(i), closestWPs.at(
          j), globalPath, path, tempCurrentForwardPathss);
  ...
  if (bDirectionBased && bFindBranches) {
    ...
    PlanningHelpers::CreateManualBranchFromTwoPoints(p1, p2, branch_length, FORWARD_RIGHT_DIR,
      r_branch);
    PlanningHelpers::CreateManualBranchFromTwoPoints(p1, p2, branch_length, FORWARD_LEFT_DIR,
      l_branch);
...
double PlannerH::PredictPlanUsingDP(
  const WayPoint & startPose, WayPoint * closestWP,
  const double & maxPlanningDistance,
  std::vector<std::vector<WayPoint>> & paths,
  const bool & bFindBranches)
{
  ...
  vector<WayPoint *> pLaneCells;
  int nPaths = PlanningHelpers::PredictiveDP(closestWP, maxPlanningDistance, all_cell_to_delete,
      pLaneCells);
  ...
  for (unsigned int i = 0; i < pLaneCells.size(); i++) {
    std::vector<WayPoint> path;
    PlanningHelpers::TraversePathTreeBackwards(pLaneCells.at(
        i), closestWP, globalPath, path, tempCurrentForwardPathss);
    if (path.size() > 0) {
      totalPlanDistance += path.at(path.size() - 1).cost;
      path.insert(path.begin(), startPose);
      //path.at(0).pos.a = path.at(1).pos.a;;
