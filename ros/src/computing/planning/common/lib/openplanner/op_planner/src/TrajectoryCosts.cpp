...
void TrajectoryCosts::CalculateLateralAndLongitudinalCostsStatic(vector<TrajectoryCost>& trajectoryCosts,
...
void TrajectoryCosts::CalculateIntersectionVelocities(const std::vector<PlannerHNS::WayPoint>& path, const PlannerHNS::DetectedObject& obj, const WayPoint& currPose, const CAR_BASIC_INFO& carInfo, const double& c_lateral_d, WayPoint& collisionPoint, TrajectoryCost& trajectoryCosts)
{
		...
		for(unsigned int j = 0; j < obj.predTrajectories.at(k).size(); j++)
		{
			for(unsigned int i = 0; i < path.size(); i++)
			{
				//if(path.at(i).timeCost > -1)
				{
					...
					if(collision_distance <= c_lateral_d  && fabs(path.at(i).timeCost - obj.predTrajectories.at(k).at(j).timeCost) < m_CollisionTimeDiff)
					{
						std::cout << "Collision Time: " << obj.predTrajectories.at(k).at(j).timeCost << std::endl;
...
int TrajectoryCosts::GetCurrentRollOutIndex(const std::vector<WayPoint>& path, const WayPoint& currState, const PlanningParams& params)
{
	RelativeInfo obj_info;
...
void TrajectoryCosts::InitializeCosts(const vector<vector<WayPoint> >& rollOuts, const PlanningParams& params)
{
	m_TrajectoryCosts.clear();
	if(rollOuts.size()>0)
	{
...
void TrajectoryCosts::InitializeSafetyPolygon(const WayPoint& currState, const CAR_BASIC_INFO& carInfo, const VehicleState& vehicleState, const double& c_lateral_d, const double& c_long_front_d, const double& c_long_back_d)
{
	PlannerHNS::Mat3 invRotationMat(currState.pos.a-M_PI_2);
	PlannerHNS::Mat3 invTranslationMat(currState.pos.x, currState.pos.y);
...
void TrajectoryCosts::CalculateLateralAndLongitudinalCostsDynamic(const std::vector<PlannerHNS::DetectedObject>& obj_list, const vector<vector<WayPoint> >& rollOuts, const vector<WayPoint>& totalPaths,
