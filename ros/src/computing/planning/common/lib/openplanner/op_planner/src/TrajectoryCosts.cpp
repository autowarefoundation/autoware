...
//	cout << "------ Normalizing Step " << endl;
  for (unsigned int ic = 0; ic < trajectoryCosts.size(); ic++) {
    ...
    trajectoryCosts.at(ic).transition_cost = m_WeightTransition *
      trajectoryCosts.at(ic).transition_cost;
    trajectoryCosts.at(ic).lane_change_cost = m_WeightLaneChange *
      trajectoryCosts.at(ic).lane_change_cost;
    trajectoryCosts.at(ic).lateral_cost = m_WeightLat * trajectoryCosts.at(ic).lateral_cost;
    trajectoryCosts.at(ic).longitudinal_cost = m_WeightLong *
...
void TrajectoryCosts::CalculateTransitionCosts(vector<TrajectoryCost> & trajectoryCosts,
