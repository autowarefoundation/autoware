...
public:
  ...
  TrajectoryCost DoOneStepDynamic(
    const vector<vector<WayPoint>> & rollOuts, const vector<WayPoint> & totalPaths,
    const WayPoint & currState, const PlanningParams & params, const CAR_BASIC_INFO & carInfo,
    const VehicleState & vehicleState,
    const std::vector<PlannerHNS::DetectedObject> & obj_list, const int & iCurrentIndex = -1);
...
private:
  ...
  void CalculateLateralAndLongitudinalCosts(
    ...
    const vector<WayPoint> & contourPoints,
    const PlanningParams & params,
  ...
  void CalculateTransitionCosts(
    vector<TrajectoryCost> & trajectoryCosts,
    const int & currTrajectoryIndex, const PlanningParams & params);

  void CalculateIntersectionVelocities(
    const std::vector<WayPoint> & path,
    const DetectedObject & obj, const WayPoint & currPose,
    const CAR_BASIC_INFO & carInfo, const double & c_lateral_d,
    WayPoint & collisionPoint, TrajectoryCost & trajectoryCosts);
  int GetCurrentRollOutIndex(
