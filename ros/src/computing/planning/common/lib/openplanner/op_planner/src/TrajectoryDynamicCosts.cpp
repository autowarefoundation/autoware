  ...
  const std::vector<PlannerHNS::DetectedObject> & obj_list, const int & iCurrentIndex)
{
  ...
  double critical_long_back_distance = carInfo.length / 2.0 + params.verticalSafetyDistance -
  ...
  InitializeSafetyPolygon(currState, carInfo, vehicleState, critical_lateral_distance,
    critical_long_front_distance, critical_long_back_distance);

  CalculateTransitionCosts(m_TrajectoryCosts, currIndex, params);

  CalculateLateralAndLongitudinalCostsDynamic(obj_list, rollOuts, totalPaths, currState, params,
    carInfo, vehicleState, critical_lateral_distance,
  ...
  const std::vector<PlannerHNS::DetectedObject> & obj_list, const int & iCurrentIndex)
{
  ...
  CalculateLateralAndLongitudinalCostsStatic(m_TrajectoryCosts, rollOuts, totalPaths, currState,
    m_AllContourPoints, params, carInfo, vehicleState);
  ...
  const std::vector<PlannerHNS::DetectedObject> & obj_list)
{
  ...
  CalculateLateralAndLongitudinalCosts(m_TrajectoryCosts, rollOuts, totalPaths, currState,
    m_AllContourPoints, params, carInfo, vehicleState);
  ...
  const CAR_BASIC_INFO & carInfo, const VehicleState & vehicleState)
{
  ...
  GPSPoint bottom_left(-critical_lateral_distance, -critical_long_back_distance, currState.pos.z,
    0);
  GPSPoint bottom_right(critical_lateral_distance, -critical_long_back_distance, currState.pos.z,
    0);

  GPSPoint top_right_car(critical_lateral_distance, carInfo.wheel_base / 3.0 + carInfo.length / 3.0,
    currState.pos.z, 0);
  GPSPoint top_left_car(-critical_lateral_distance, carInfo.wheel_base / 3.0 + carInfo.length / 3.0,
    currState.pos.z, 0);

  GPSPoint top_right(critical_lateral_distance - slide_distance, critical_long_front_distance,
    currState.pos.z, 0);
  GPSPoint top_left(-critical_lateral_distance - slide_distance, critical_long_front_distance,
    currState.pos.z, 0);
  ...
  if (rollOuts.size() > 0 && rollOuts.at(0).size() > 0) {
    ...
    for (unsigned int it = 0; it < rollOuts.size(); it++) {
      ...
      for (unsigned int icon = 0; icon < contourPoints.size(); icon++) {
        ...
        double longitudinalDist = PlanningHelpers::GetExactDistanceOnTrajectory(totalPaths,
            car_info, obj_info);
        if (obj_info.iFront == 0 && longitudinalDist > 0) {
          longitudinalDist = -longitudinalDist;
        }

        double direct_distance = hypot(obj_info.perp_point.pos.y - contourPoints.at(
              icon).pos.y, obj_info.perp_point.pos.x - contourPoints.at(icon).pos.x);
        if (contourPoints.at(icon).v < params.minSpeed &&
          direct_distance > (m_LateralSkipDistance + contourPoints.at(icon).cost)) {
...
//					if(close_in_percentage <= 0 || close_in_percentage > 1) close_in_percentage = 1;
        ...
        if (longitudinalDist >= -critical_long_front_distance &&
          longitudinalDist < trajectoryCosts.at(iCostIndex).closest_obj_distance) {
    ...
    if (rollOuts.at(il).size() > 0 && rollOuts.at(il).at(0).size() > 0) {
      ...
      for (unsigned int it = 0; it < rollOuts.at(il).size(); it++) {
        ...
        for (unsigned int icon = 0; icon < contourPoints.size(); icon++) {
          ...
          double longitudinalDist = PlanningHelpers::GetExactDistanceOnTrajectory(totalPaths.at(
                il), car_info, obj_info);
          if (obj_info.iFront == 0 && longitudinalDist > 0) {
            longitudinalDist = -longitudinalDist;
          }
...
//					if(close_in_percentage <= 0 || close_in_percentage > 1) close_in_percentage = 1;
          ...
          if (m_SafetyBorder.PointInsidePolygon(m_SafetyBorder,
            contourPoints.at(icon).pos) == true) {
            trajectoryCosts.at(iCostIndex).bBlocked = true;
          }
...
//	cout << "------ Normalizing Step " << endl;
  for (unsigned int ic = 0; ic < trajectoryCosts.size(); ic++) {
    ...
    if (transitionCosts != 0) {
      trajectoryCosts.at(ic).transition_cost = trajectoryCosts.at(ic).transition_cost /
        transitionCosts;
    } else {
    ...
    if (totalChange != 0) {
      trajectoryCosts.at(ic).lane_change_cost = trajectoryCosts.at(ic).lane_change_cost /
        totalChange;
    } else {
    ...
    if (totalLongitudinalCosts != 0) {
      trajectoryCosts.at(ic).longitudinal_cost = trajectoryCosts.at(ic).longitudinal_cost /
    ...
    trajectoryCosts.at(ic).cost =
      (m_WeightPriority * trajectoryCosts.at(ic).priority_cost + m_WeightTransition *
      trajectoryCosts.at(ic).transition_cost + m_WeightLat * trajectoryCosts.at(ic).lateral_cost +
      m_WeightLong * trajectoryCosts.at(ic).longitudinal_cost) / 4.0;
...
vector<TrajectoryCost> TrajectoryDynamicCosts::CalculatePriorityAndLaneChangeCosts(
  const vector<vector<WayPoint>> & laneRollOuts,
...
void TrajectoryDynamicCosts::CalculateTransitionCosts(vector<TrajectoryCost> & trajectoryCosts,
...
void TrajectoryDynamicCosts::CalculateIntersectionVelocities(
  const std::vector<PlannerHNS::WayPoint> & path, const PlannerHNS::DetectedObject & obj,
  const WayPoint & currPose, const CAR_BASIC_INFO & carInfo, const double & c_lateral_d,
  ...
  for (unsigned int k = 0; k < obj.predTrajectories.size(); k++) {
    for (unsigned int j = 0; j < obj.predTrajectories.at(k).size(); j++) {
      for (unsigned int i = 0; i < path.size(); i++) {
        ...
        {
          double collision_distance = hypot(path.at(i).pos.x - obj.predTrajectories.at(k).at(
                j).pos.x, path.at(i).pos.y - obj.predTrajectories.at(k).at(j).pos.y);
          double collision_t =
            fabs(path.at(i).timeCost - obj.predTrajectories.at(k).at(j).timeCost);
...
int TrajectoryDynamicCosts::GetCurrentRollOutIndex(const std::vector<WayPoint> & path,
...
void TrajectoryDynamicCosts::InitializeCosts(const vector<vector<WayPoint>> & rollOuts,
  const PlanningParams & params)
{
  m_TrajectoryCosts.clear();
  if (rollOuts.size() > 0) {
...
void TrajectoryDynamicCosts::InitializeSafetyPolygon(const WayPoint & currState,
  ...
  const double & c_long_back_d)
{
  ...
  GPSPoint top_right_car(c_lateral_d, carInfo.wheel_base / 3.0 + carInfo.length / 3.0,
    currState.pos.z, 0);
  GPSPoint top_left_car(-c_lateral_d, carInfo.wheel_base / 3.0 + carInfo.length / 3.0,
    currState.pos.z, 0);
  ...
  const double & c_long_back_d)
{
  ...
  for (unsigned int i = 0; i < obj_list.size(); i++) {
    if (obj_list.at(i).label.compare("curb") == 0) {
      double d = hypot(obj_list.at(i).center.pos.y - currState.pos.y, obj_list.at(
            i).center.pos.x - currState.pos.x);
    ...
    if (obj_list.at(i).bVelocity && obj_list.at(i).predTrajectories.size() > 0) {          // dynamic
      ...
      for (unsigned int ir = 0; ir < rollOuts.size(); ir++) {
        ...
        CalculateIntersectionVelocities(rollOuts.at(ir), obj_list.at(
            i), currState, carInfo, c_lateral_d, collisionPoint, trajectoryCosts);
      ...
      for (unsigned int icon = 0; icon < obj_list.at(i).contour.size(); icon++) {
        if (m_SafetyBorder.PointInsidePolygon(m_SafetyBorder,
          obj_list.at(i).contour.at(icon)) == true) {
          for (unsigned int it = 0; it < rollOuts.size(); it++) {
        ...
        for (unsigned int it = 0; it < rollOuts.size(); it++) {
          double lateralDist = fabs(obj_info.perp_distance - m_TrajectoryCosts.at(
                it).distance_from_center);

          if (lateralDist > m_LateralSkipDistance) {
            continue;
          }

          if (lateralDist <= c_lateral_d && longitudinalDist > -carInfo.length &&
            longitudinalDist < params.minFollowingDistance) {
            m_TrajectoryCosts.at(it).bBlocked = true;
          ...
          if (longitudinalDist >= -c_long_front_d &&
            longitudinalDist < m_TrajectoryCosts.at(it).closest_obj_distance) {
