...
double _downsampler_res = G_MAP_CELLSIZE;

int _map_size_x = G_MAP_X;
int _map_size_y = G_MAP_Y;
int _map_size_z = G_MAP_Z;
...
int main(int argc, char *argv[])
{
  ...
  private_nh.getParam("downsampler_res", _downsampler_res);
  ...
  private_nh.getParam("map_size_x", _map_size_x);
  private_nh.getParam("map_size_y", _map_size_y);
  private_nh.getParam("map_size_z", _map_size_z);

  std::cout << "Downsampler: " << _downsampler << std::endl;
  if (_downsampler == "voxel_grid")
  {
     std::cout << "Downsampler is: \"" << _downsampler << "\" with resolution of " << _downsampler_res << std::endl;
     //g_map_cellsize = _downsampler_res;
    _downsampler_num = 1;
  }
  if (_downsampler == "distance")
  {
