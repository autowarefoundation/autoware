...
void velodyneCallback(const pcl::PointCloud<pcl::PointXYZI>::ConstPtr& msg)
{
  ...
  */
  ...
  
  {
	...
	if(scan_points[j].z>-1&&scan_points[j].z<0&&
	   scan_points[j].y>-20&&scan_points[j].y<20)continue;
	...
	double dist=scan_points[j].x*scan_points[j].x
	  ...
	  +scan_points[j].z*scan_points[j].z;
	if(dist<3*3)continue;
	scan_points_weight[j]=(dist)* (1.0-0.5*exp(-1*(-0.5 - scan_points[j].z)*(-0.5 - scan_points[j].z)/2.0));
	scan_points_totalweight+=scan_points_weight[j];
...
void load (char *name){
  ...
  while(fscanf(fp,"%lf %lf %lf",&x,&y,&z)!=EOF){q=100;//for xyz file
