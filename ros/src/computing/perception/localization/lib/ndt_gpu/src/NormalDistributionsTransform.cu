...
//copied code from Registration.cu
template <typename T>
...
double GNormalDistributionsTransform::getFitnessScore(pcl::PointCloud<pcl::PointXYZ>::Ptr input, int* const nr, double max_range)
{
	double fitness_score = 0.0;

	int points_number = input->points.size();

	pcl::PointXYZ *tmp;

	checkCudaErrors(cudaMalloc(&tmp, sizeof(pcl::PointXYZ) * points_number));

	pcl::PointXYZ *host_tmp = input->points.data();

	// Pin the host buffer for accelerating the memory copy
#ifndef __aarch64__
	checkCudaErrors(cudaHostRegister(host_tmp, sizeof(pcl::PointXYZ) * points_number, cudaHostRegisterDefault));
#endif

	checkCudaErrors(cudaMemcpy(tmp, host_tmp, sizeof(pcl::PointXYZ) * points_number, cudaMemcpyHostToDevice));

	float *input_x, *input_y, *input_z;

	checkCudaErrors(cudaMalloc(&input_x, sizeof(float) * points_number));
	checkCudaErrors(cudaMalloc(&input_y, sizeof(float) * points_number));
	checkCudaErrors(cudaMalloc(&input_z, sizeof(float) * points_number));

	int block_x_tmp = (points_number > BLOCK_SIZE_X) ? BLOCK_SIZE_X : points_number;
	int grid_x_tmp = (points_number - 1) / block_x_tmp + 1;

	convertInput<pcl::PointXYZ><<<grid_x_tmp, block_x_tmp>>>(tmp, input_x, input_y, input_z, points_number);
	checkCudaErrors(cudaGetLastError());
	checkCudaErrors(cudaDeviceSynchronize());
	...
	checkCudaErrors(cudaMalloc(&trans_x, sizeof(float) * points_number));
	checkCudaErrors(cudaMalloc(&trans_y, sizeof(float) * points_number));
	checkCudaErrors(cudaMalloc(&trans_z, sizeof(float) * points_number));

	transformPointCloud(input_x, input_y, input_z, trans_x, trans_y, trans_z, points_number, final_transformation_);

	int *valid_distance;

	checkCudaErrors(cudaMalloc(&valid_distance, sizeof(int) * points_number));

	double *min_distance;

	checkCudaErrors(cudaMalloc(&min_distance, sizeof(double) * points_number));

	voxel_grid_.nearestNeighborSearch(trans_x, trans_y, trans_z, points_number, valid_distance, min_distance, max_range);

	int size = points_number;
	int half_size;
	...
	checkCudaErrors(cudaFree(input_x));
	checkCudaErrors(cudaFree(input_y));
	checkCudaErrors(cudaFree(input_z));
	checkCudaErrors(cudaFree(trans_x));
	checkCudaErrors(cudaFree(trans_y));
	checkCudaErrors(cudaFree(trans_z));
	checkCudaErrors(cudaFree(valid_distance));
...
double GNormalDistributionsTransform::getFitnessScore(pcl::PointCloud<pcl::PointXYZI>::Ptr input, int* const nr, double max_range)
{
	double fitness_score = 0.0;
	...
	checkCudaErrors(cudaMalloc(&tmp, sizeof(pcl::PointXYZI) * points_number));

	pcl::PointXYZI *host_tmp = input->points.data();

	// Pin the host buffer for accelerating the memory copy
#ifndef __aarch64__
	checkCudaErrors(cudaHostRegister(host_tmp, sizeof(pcl::PointXYZI) * points_number, cudaHostRegisterDefault));
#endif

	checkCudaErrors(cudaMemcpy(tmp, host_tmp, sizeof(pcl::PointXYZI) * points_number, cudaMemcpyHostToDevice));
	...
	convertInput<pcl::PointXYZI><<<grid_x_tmp, block_x_tmp>>>(tmp, input_x, input_y, input_z, points_number);
	checkCudaErrors(cudaGetLastError());
	checkCudaErrors(cudaDeviceSynchronize());
