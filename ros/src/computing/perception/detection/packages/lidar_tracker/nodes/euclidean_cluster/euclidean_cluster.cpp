...
std::string _using_gridmap_layer;
...
std::map<std::string, std::vector<std::vector<geometry_msgs::Point>>> _area_points;
...
void convertPointsToImage(grid_map::GridMap& out_grid_map, const std::string& layer_name,
                          const std::vector<std::vector<geometry_msgs::Point>>& in_wayarea_points)
{
  if (!out_grid_map.exists(layer_name))
  {
    out_grid_map.add(layer_name);
  }
  out_grid_map[layer_name].setConstant(100);  // fill with gray color

  cv::Mat original_image;
  grid_map::GridMapCvConverter::toImage<unsigned char, 1>(out_grid_map, layer_name, CV_8UC1, 0, 100, original_image);
  ...
  // convert to ROS msg
  grid_map::GridMapCvConverter::addLayerFromImage<unsigned char, 1>(filled_image, layer_name, out_grid_map, 0, 100);
}
                             ...
                             const std::string& map_class)
{
  ...
  _area_points[map_class].push_back(area_points);
...
void refillGridMap(grid_map::GridMap& out_grid_map, const std::string& layer_name)
{
  if (!out_grid_map.exists(layer_name))
  {
    out_grid_map.add(layer_name);
                       ...
                       jsk_rviz_plugins::PictogramArray& in_out_pictogram_array)
{
  ...
  for (unsigned int i = 0; i < cloud_segments_array.size(); i++)
  {
    ...
    std::vector<ClusterPtr> local_clusters = clusterAndColor(
        cloud_segments_array[i], out_cloud_ptr, in_out_boundingbox_array, in_out_centroids, _clustering_thresholds[i]);
#endif
  ...
  if (_use_vector_map)
  {
    ...
    if (_wayarea_gridmap != NULL)
    {
      for (auto& layer : _wayarea_gridmap->getLayers())
      {
        refillGridMap(*_wayarea_gridmap, layer);
        // 	_wayarea_gridmap[layer]->setConstant(100);
      }
    }
    if (_wayarea_gridmap != NULL && _wayarea_gridmap->exists(_using_gridmap_layer))
    {
      _wayarea_gridmap->setFrameId(_velodyne_header.frame_id);
      convertPointsToImage(*_wayarea_gridmap, _using_gridmap_layer, _area_points[_using_gridmap_layer]);
      ...
      cv::Mat grid_image;
      grid_map::GridMapCvConverter::toImage<unsigned char, 1>(*_wayarea_gridmap, _using_gridmap_layer, CV_8UC1, 0, 100,
                                                              grid_image);
...
void gridmap_layer_callback(const std_msgs::String& layer_name)
{
  _using_gridmap_layer = layer_name.data;
  ROS_INFO("using gridmap layer is %s", _using_gridmap_layer.c_str());
}
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
  ...
  double elapsed =
      std::chrono::duration_cast<std::chrono::milliseconds>(_end - _start).count();  //処理に要した時間をミリ秒に変換
...
int main(int argc, char** argv)
{
  ...
  private_nh.param("vectormap_grid_width", _vectormap_grid_width, 30.0);
  ROS_INFO("vectormap_grid_width: %f", _vectormap_grid_width);
  private_nh.param("vectormap_grid_height", _vectormap_grid_height, 70.0);
  ROS_INFO("vectormap_grid_height: %f", _vectormap_grid_height);
  private_nh.param("vectormap_grid_resolution", _vectormap_grid_resolution, 0.3);
  ROS_INFO("vectormap_grid_resolution: %f", _vectormap_grid_resolution);
  private_nh.param("vectormap_grid_behind", _vectormap_grid_behind, 20.0);
  ROS_INFO("vectormap_grid_resolution: %f", _vectormap_grid_behind);
  ...
  for (auto i = _clustering_thresholds.begin(); i != _clustering_thresholds.end(); ++i)
    std::cout << *i << ' ';
  std::cout << std::endl;
  std::cout << "_clustering_distances: ";
  for (auto i = _clustering_distances.begin(); i != _clustering_distances.end(); ++i)
    std::cout << *i << ' ';
  std::cout << std::endl;

  // Create a ROS subscriber for the input point cloud
  ros::Subscriber sub = h.subscribe(points_topic, 1, velodyne_callback);
  ros::Subscriber sub_1 = h.subscribe("/state/filtering_gridmap_layer", 1, gridmap_layer_callback);
  ...
  std::vector<vector_map_msgs::WayArea> way_areas =
      vector_map.findByFilter([](const vector_map_msgs::WayArea& way_area) { return true; });
  std::vector<vector_map_msgs::DetectionArea> detection_areas =
      vector_map.findByFilter([](const vector_map_msgs::DetectionArea& detection_area) { return true; });

  grid_map::GridMap gridmap({ "wayarea", "detectionarea" });
  _wayarea_gridmap = &gridmap;
  _wayarea_gridmap->setGeometry(grid_map::Length(_vectormap_grid_height, _vectormap_grid_width),
                                _vectormap_grid_resolution);
  _wayarea_gridmap->setPosition(grid_map::Position(_vectormap_grid_behind, 0));

  if (way_areas.empty())
  {
    ROS_WARN_STREAM("[euclidean_cluster]: The VectorMap is not being published or does not contain WAYAREA, not "
                    "performing filtering.");
    _wayarea_gridmap = NULL;
  }
  else if (detection_areas.empty())
  {
    ROS_WARN_STREAM("[euclidean_cluster]: The VectorMap is not being published or does not contain DETECTION_AREA, not "
                    "performing filtering by DETECTION_AREA.");
  }
  else
  {
    ROS_INFO("[euclidean_cluster]: Found %lu wayareas.", way_areas.size());
    ROS_INFO("[euclidean_cluster]: Found %lu detectionareas.", detection_areas.size());
    for (const auto& way_area : way_areas)
    {
      vector_map::Area area = vector_map.findByKey(vector_map::Key<vector_map::Area>(way_area.aid));
      getWayAreaPointsFromMap(area, vector_map, "wayarea");
    }
    for (const auto& detection_area : detection_areas)
    {
      vector_map::Area area = vector_map.findByKey(vector_map::Key<vector_map::Area>(detection_area.aid));
      getWayAreaPointsFromMap(area, vector_map, "detectionarea");
    }
