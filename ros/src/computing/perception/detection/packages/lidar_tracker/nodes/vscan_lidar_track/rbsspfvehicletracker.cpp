...
void RBSSPFVehicleTrackerInstance::slotCheckInitState(int initNum, VehicleState * initState, bool * initFlag)
{
    for(int i=0;i<initNum;i++)
    {
        ...
        double dx=tmpdx*cos(-tracker_result_container_.estimate.theta)-tmpdy*sin(-tracker_result_container_.estimate.theta);
        double dy=tmpdx*sin(-tracker_result_container_.estimate.theta)+tmpdy*cos(-tracker_result_container_.estimate.theta);
...
void RBSSPFVehicleTracker::addTrackerData(LaserScan & scan, QVector<VehicleState> & initState)
{
    ...
    case ReadyForTracking:
        ...
        if(detection_num>0)//if there are detections
        {
            ...
            {
                emit signalCheckInitState(detection_num,detections_.data(),detections_matched_.data());
            }
        ...
        {
            tracker_state_=ReadyForTracking;
            emit signalUpdateTrackerFinish(current_laserscan_,tracker_result_container_map_);
        }
        break;
    case Processing:
    ...
    if(tracker_count_<=0)
    {
        ...
        for(i=0;i<n;i++)
        {
            if(detections_matched_[i])//not matched
            {
                ...
                RBSSPFVehicleTrackerInstance * trackerinstance=new RBSSPFVehicleTrackerInstance(tracker_id_,thread);
                tracker_result_container_map_.insert(tracker_id_,TrackerResultContainer());
