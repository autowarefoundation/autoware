...
{
  const double tipSize = cv::norm(in_point1 - in_point2) *
                         in_tip_length;  // Factor to normalize the size of the tip depending on the length of the arrow
...
cv::Mat LkTracker::Track(cv::Mat in_image, cv::LatentSvmDetector::ObjectDetection in_detection, bool in_update)
{
  ...
  if (in_update && in_detection.rect.width > 0)
  {
    ...
    if (matched_detection_.rect.x + matched_detection_.rect.width > in_image.cols)
      matched_detection_.rect.width = in_image.cols - matched_detection_.rect.x;
    if (matched_detection_.rect.y + matched_detection_.rect.height > in_image.rows)
      matched_detection_.rect.height = in_image.rows - matched_detection_.rect.y;
      ...
      (matched_detection_.rect.width > 0 && matched_detection_.rect.height > 0))  // add as new object
  {
    ...
    cv::cornerSubPix(gray_image, current_points_, sub_pixel_window_size_, cv::Size(-1, -1), term_criteria_);
    // frame_count_ = 0;
    current_centroid_x_ = 0;
    current_centroid_y_ = 0;
  ...
  else if (!prev_points_.empty())  // try to match current object
  {
    ...
    cv::calcOpticalFlowPyrLK(prev_image_,  // previous image frame
                             ...
                             status, err, window_size_, 3, term_criteria_, 0, 0.001);
    std::size_t i = 0, k = 0;
  ...
  cv::kmeans(valid_points, 2, labels, cv::TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 10, 1.0), 3,
  ...
  for (std::size_t i = 0; i < valid_points.size(); i++)
  {
    ...
    if (cv::norm(valid_points[i] - center1) < matched_detection_.rect.width * 0.8 &&
        cv::norm(valid_points[i] - center2) <
            matched_detection_.rect.width * 0.8)  // distance between point and cluster centroid
    {
  ...
  if (cv::norm(center2 - center1) >
      matched_detection_.rect.width * 0.75)  // if centroids are too far keep only the one with the most points
  {
  ...
  {
    cv::Point center_point = cv::Point(current_rect_.rect.x + current_rect_.rect.width / 2,
...
void LkTracker::GetRectFromPoints(std::vector<cv::Point2f> in_corners_points, cv::Rect& out_boundingbox)
{
  ...
  int min_x = in_corners_points[0].x, min_y = in_corners_points[0].y, max_x = in_corners_points[0].x,
      max_y = in_corners_points[0].y;
