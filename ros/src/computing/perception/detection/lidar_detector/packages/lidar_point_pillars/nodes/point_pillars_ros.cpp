...
geometry_msgs::Pose PointPillarsROS::getTransformedPose(const geometry_msgs::Pose& in_pose,
...
void PointPillarsROS::pubDetectedObject(const std::vector<float>& detections, const int num_objects, const std_msgs::Header& pc_header)
{
...
void PointPillarsROS::pclToArray(const pcl::PointCloud<pcl::PointXYZI>::Ptr & in_pcl_pc_ptr, float* out_points_array, const float offset_z)
{
...
void PointPillarsROS::pointsCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
  ...
  point_pillars_ptr_->doInference(points_array, pcl_pc_ptr->size(), out_detection, out_num_objects);

  delete[] points_array;
  pubDetectedObject(out_detection, out_num_objects, msg->header);
}
