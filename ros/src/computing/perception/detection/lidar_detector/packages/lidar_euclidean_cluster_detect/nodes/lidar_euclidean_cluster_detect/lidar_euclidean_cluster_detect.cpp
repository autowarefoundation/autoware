                       ...
                       jsk_rviz_plugins::PictogramArray &in_out_pictogram_array)
{
    ...
    if (!_use_multiple_thres)
    {
        ...
        {
            all_clusters =
                    clusterAndColor(cloud_ptr, out_cloud_ptr, in_out_boundingbox_array, in_out_centroids,
                                    _clustering_distance);
        }
#else
        all_clusters =
...
int main(int argc, char **argv)
{
    ...
    _pub_jsk_boundingboxes = h.advertise<jsk_recognition_msgs::BoundingBoxArray>(
            "/detection/lidar_detector/objects_boxes", 1);
    _pub_jsk_hulls = h.advertise<jsk_recognition_msgs::PolygonArray>("/detection/lidar_detector/objects_hulls", 1);
    _pub_clusters_message = h.advertise<autoware_msgs::CloudClusterArray>("/detection/lidar_detector/objects_clusters",
                                                                          1);
    _pub_detected_objects = h.advertise<autoware_msgs::DetectedObjectArray>("/detection/lidar_detector/objects", 1);
    _pub_text_pictogram = h.advertise<jsk_rviz_plugins::PictogramArray>("/detection/lidar_detector/objects_pictograms",
                                                                        10);
    ROS_INFO("[%s] output pictograms topic: %s", __APP_NAME__, "cluster_id");
    ...
    {
        ROS_INFO("[%s] euclidean_cluster > Setting points node to %s", __APP_NAME__, points_topic.c_str());
    } else
    {
        ROS_INFO("[%s] euclidean_cluster > No points node received, defaulting to points_raw, you can use "
                         "_points_node:=YOUR_TOPIC", __APP_NAME__);
    ...
    {
        if (_use_diffnormals)
            ROS_INFO("[%s] Euclidean Clustering: Applying difference of normals on clustering pipeline", __APP_NAME__);
        else
            ROS_INFO("[%s] Euclidean Clustering: Difference of Normals will not be used.", __APP_NAME__);
    }
    ...
    if (_use_multiple_thres)
    {
        ...
        {
            ROS_ERROR("[%s] Invalid size of clustering_ranges or/and clustering_distance. \
    The size of clustering distance and clustering_ranges should not be 0", __APP_NAME__);
            ros::shutdown();
        }
        if ((distances_size - ranges_size) != 1)
        {
            ROS_ERROR("[%s]Invalid size of clustering_ranges or/and clustering_distance. \
    Expecting that (distances_size - ranges_size) == 1 ", __APP_NAME__);
            ros::shutdown();
        }
