...
public:
  ...
  static ObjectCuboid params_carCuboid;
  ...
  double cd_area_;
  cv::Mat cd_2d_;
  vector<Point> pd_2d_convhull_;
  Paths pd_2d_convhull_clip_;
  Paths cd_2d_convhull_clip_;

  vector<Point> pd_3d_convhull_;
  vector<Point> cd_3d_convhull_;
  ...
  void propagate_cur_det(cv::Mat cuboid, double h, cv::Mat k, cv::Mat inv_k, cv::Mat n) {
    ...
    cv::Mat b1Q = (cv::Mat_<double>(3, 1) << bbox[0] + (bbox[2] - bbox[0]) / 2, bbox[3], 1.0);
    ...
    cv::Mat bvolume = get_boundingbox_volume(cuboid, yaw); //translated_cuboid);
    origin_ = B1Q;
    cv::Mat offset = (cv::Mat_<double>(1, 3) << 0, params_carCuboid.avg_w / 2., 0);
    ...
    cd_area_ = (bbox[3] - bbox[1]) * (bbox[2] - bbox[0]);
  }
...
private:
  ...
  double calc_offset_base_yaw() {
    double offset;
    double l = params_carCuboid.avg_l / 2.;
    double w = params_carCuboid.avg_w / 2.;
    cv::Mat car_bottom_plane = (cv::Mat_<double>(4, 2) << -l, -w, l, -w, l, w, -l, w);
    car_bottom_plane = car_bottom_plane.t();

    cv::Mat rot = (cv::Mat_<double>(2, 2) << cos(yaw), -sin(yaw), sin(yaw), cos(yaw));
    ...
    offset = abs(car_bottom_plane_rot.at<double>(max_loc.y, 1) - car_bottom_plane_rot.at<double>(min_loc.y, 1)) / 2.;
    return offset;
  }
  ...
  vector<Point> get_convhull(cv::Mat mat_2d) {
    int size_pd = mat_2d.size().height;
    vector<cv::Point> mat_2d_convhull(1);
