...
extern int getMaxFilterDims(const CvLSVMFilterObject **filters, int kComponents, const int *kPartFilters,
                            unsigned int *maxXBorder, unsigned int *maxYBorder);
extern int clippingBoxes(int width, int height, CvPoint *points, int kPoints);
extern int nonMaximumSuppression(int numBoxes, const CvPoint *points, const CvPoint *oppositePoints, const float *score,
                                 float overlapThreshold, int *numBoxesOut, CvPoint **pointsOut,
                                 CvPoint **oppositePointsOut, float **scoreOut);
extern int loadModel(const char *modelPath, CvLSVMFilterObject ***filters, int *kFilters, int *kComponents,
                     int **kPartFilters, float **b, float *scoreThreshold);
};
...
static int convertPoints(int /*countLevel*/, int lambda, int initialImageLevel, CvPoint *points, int *levels,
                         CvPoint **partsDisplacement, int kPoints, int n, int maxXBorder, int maxYBorder)
{
  int i, j, bx, by;
  float step, scale;
 ...
 */
static CvLatentSvmDetector *cvLoadLatentSvmDetector(const char *filename, const float scoreThreshold)
{
...
#endif
  err_code = loadModel(filename, &filters, &kFilters, &kComponents, &kPartFilters, &b, &modelsScoreThreshold);
#ifdef PROFILE
  tm.stop();
...
bool DPMOCVGPULatentSvmDetector::load(const std::vector<std::string> &filenames, const float test_t)
{
  ...
  for (size_t i = 0; i < filenames.size(); i++)
  {
    ...
    CvLatentSvmDetector *detector = cvLoadLatentSvmDetector(filename.c_str(), test_t);
    if (detector)
    {
...
static CvLSVMFeaturePyramid *createFeaturePyramidWithBorder(IplImage *image, int maxXBorder, int maxYBorder)
{
  int opResult;
  int bx, by;
...
static int searchObjectThreshold(const CvLSVMFeaturePyramid *H, const CvLSVMFilterObject **all_F, int n, float b,
                                 CvLSVMFeatureMap *map[], int maxXBorder, int maxYBorder, float scoreThreshold,
                                 CvPoint **points, int **levels, int *kPoints, float **score,
                                 CvPoint ***partsDisplacement, int numThreads)
{
  int opResult;

  opResult =
      thresholdFunctionalScore(all_F, n, H, b, map, scoreThreshold, score, points, levels, kPoints, partsDisplacement);
  ...
  convertPoints(H->numLevels, Lambda, Lambda, (*points), (*levels), (*partsDisplacement), (*kPoints), n, maxXBorder,
                maxYBorder);

  return LATENT_SVM_OK;
}

static int getOppositePoint(CvPoint point, int sizeX, int sizeY, float step, int degree, CvPoint *oppositePoint)
{
  float scale;
...
static int searchObjectThresholdSomeComponents(const CvLSVMFeaturePyramid *H, const CvLSVMFilterObject **filters,
                                               int kComponents, const int *kPartFilters, const float *b,
                                               float scoreThreshold, CvPoint **points, CvPoint **oppPoints,
                                               float **score, int *kPoints, int numThreads)
{
  ...
  {
    map[i] = featureMapBorderPartFilter(H->pyramid[i], maxXBorder, maxYBorder);
  }
  ...
  *kPoints = 0;
  ...
  {
    int error = searchObjectThreshold(H, &(filters[componentIndex]), kPartFilters[i], b[i], map, maxXBorder, maxYBorder,
                                      scoreThreshold, &(pointsArr[i]), &(levelsArr[i]), &(kPointsArr[i]),
    ...
    estimateBoxes(pointsArr[i], levelsArr[i], kPointsArr[i], filters[componentIndex]->sizeX,
                  filters[componentIndex]->sizeY, &(oppPointsArr[i]));
...
static CvSeq *cvLatentSvmDetectObjectsGPU(IplImage *image, CvLatentSvmDetector *detector, CvMemStorage *storage,
                                          float overlap_threshold, int numThreads)
{
  ...
  error = searchObjectThresholdSomeComponents(
      H, (const CvLSVMFilterObject **)(detector->filters), detector->num_components, detector->num_part_filters,
      detector->b, detector->score_threshold, &points, &oppPoints, &score, &kPoints, numThreads);
  ...
  // NMS procedure
  nonMaximumSuppression(kPoints, points, oppPoints, score, overlap_threshold, &numBoxesOut, &pointsOut, &oppPointsOut,
                        &scoreOut);
...
void DPMOCVGPULatentSvmDetector::detect(const cv::Mat &image, std::vector<ObjectDetection> &objectDetections,
                                        float overlapThreshold, int numThreads, const unsigned int lambda,
                                        const unsigned int sideLength, const float valOfTruncate)
{
  ...
  for (size_t classID = 0; classID < detectors.size(); classID++)
  {
    ...
    CvSeq *detections =
        cvLatentSvmDetectObjectsGPU(&image_ipl, detectors[classID], storage, overlapThreshold, numThreads);
