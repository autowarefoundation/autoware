...
extern int allocFeatureMapObject(CvLSVMFeatureMap **obj, const int sizeX, const int sizeY, const int p);
...
/*
 ...
 // Error status
 */
static int calculationScoreSize(const CvLSVMFilterObject *filter, const CvLSVMFeatureMap *map, int *diffX, int *diffY)
{
...
/*
 ...
 // Error status
 */
static int convolutionGPU(const CvLSVMFilterObject *filter, const CvLSVMFeatureMap *map, CUdeviceptr *dev_filter,
                          CUdeviceptr *dev_map, CUdeviceptr *dev_filterIdxTbl, CUdeviceptr *dev_score, int filterIdx,
                          CUstream stream)
{
  ...
  if (res == LATENT_SVM_OK)
  {
    ...
    {
      filterIdxTbl[i] = (i / (prm.filterX)) * prm.mapX + (i % prm.filterX);
    }
    ...
    res = cuMemcpyHtoDAsync(*dev_filterIdxTbl, filterIdxTbl, prm.filterSize * sizeof(float), stream);
    CUDA_CHECK(res, "cuMemcpyHtoDAsync(*dev_filterIdxTbl, filterIdxTbl) %zd bytes", prm.filterSize * sizeof(float));

    void *kernel_arg[] = {(void *)&filterIdx, (void *)dev_score, (void *)dev_filterIdxTbl, (void *)&prm};

    res = cuLaunchKernel(ConvolutionKernel_func[0], prm.scoreSize, 1, 1, CONV_THREAD, 1, 1, CONV_THREAD * sizeof(float),
                         stream, kernel_arg, NULL);
...
/*
 ...
 // Error status
 */
static int DistanceTransformTwoDimensionalProblemGPU(const CvLSVMFilterObject *filter, const CvLSVMFeatureMap *map,
                                                     CUdeviceptr *dev_score, CUdeviceptr *dev_distTransWork,
                                                     CUdeviceptr *dev_distTransScore, CUdeviceptr *dev_x,
                                                     CUdeviceptr *dev_y, int diffX, int diffY, CUstream stream)
{
  void *kernel_arg[] = {(void *)dev_score, (void *)&diffX, (void *)&diffY, (void *)&filter->fineFunction[0],
                        (void *)&filter->fineFunction[1], (void *)&filter->fineFunction[2],
                        (void *)&filter->fineFunction[3], (void *)dev_distTransWork, (void *)dev_distTransScore,
                        (void *)dev_x, (void *)dev_y};

  CUresult res = cuLaunchKernel(DistanceTransformTwoDimensionalProblem_func[0], 1, 1, 1, TRANS_THREAD, 1, 1, 0, stream,
                                kernel_arg, NULL);
...
/*
 ...
 // Error status
 */
static int sumScore(const CvLSVMFilterObject **all_F, const int n, const CvLSVMFeaturePyramid *H, const int level,
                    const float b, const float scoreThreshold, const CvLSVMFilterDisposition disposition[],
                    CUdeviceptr dev_score[], CvLSVMFeatureMap *map, float **score, CvPoint **points, int *kPoints,
    ...
    for (j = 0; j < diffX; j++)
    {
      ...
      for (k = 1; k <= n; k++)
      {
        if ((2 * i + all_F[k]->V.y < map->sizeY - all_F[k]->sizeY + 1) &&
            (2 * j + all_F[k]->V.x < map->sizeX - all_F[k]->sizeX + 1))
        {
          index = (2 * i + all_F[k]->V.y) * (map->sizeX - all_F[k]->sizeX + 1) + (2 * j + all_F[k]->V.x);
          sumScorePartDisposition += disposition[k - 1].score[index];
        }
      }
      root_score[i * diffX + j] = root_score[i * diffX + j] - sumScorePartDisposition + b;
    ...
    for (j = 0; j < diffX; j++)
    {
      if (root_score[i * diffX + j] > scoreThreshold)
      {
        ...
        for (k = 1; k <= n; k++)
        {
          ...
          {
            index = (2 * i + all_F[k]->V.y) * (map->sizeX - all_F[k]->sizeX + 1) + (2 * j + all_F[k]->V.x);
            (*partsDisplacement)[last][k - 1].x = disposition[k - 1].x[index];
            (*partsDisplacement)[last][k - 1].y = disposition[k - 1].y[index];
          }
        }
        last++;
      }
...
/*
 ...
 // new_map           - new map
 */
CvLSVMFeatureMap *featureMapBorderPartFilter(CvLSVMFeatureMap *map, int maxXBorder, int maxYBorder)
{
  int bx, by;
...
/*
 ...
 // Error status
 */
static void dispositionMalloc(const CvLSVMFilterObject *filter, const CvLSVMFeatureMap *map,
                              CvLSVMFilterDisposition *disposition, int *maxSize)
{
  int diffX, diffY, res;
  res = calculationScoreSize(filter, map, &diffX, &diffY);
  if (res == LATENT_SVM_OK)
  {
    ...
    disposition->score = (float *)malloc(sizeof(float) * disposition->size);
...
/*
 ...
 // none
 */
static void dispositionCpyDtoH(CUdeviceptr *dev_distTransScore, CUdeviceptr *dev_x, CUdeviceptr *dev_y,
                               CvLSVMFilterDisposition *disposition, CvLSVMFilterDisposition *tmp_disposition,
                               CUstream *stream)
{
  ...
  CUresult res =
      cuMemcpyDtoHAsync(tmp_disposition->score, *dev_distTransScore, disposition->size * sizeof(float), stream[0]);
  CUDA_CHECK(res, "cuMemcpyDtoHAsync(tmp_disposition->score, *dev_distTransScore): %zd bytes",
             disposition->size * sizeof(float));
  res = cuMemcpyDtoHAsync(tmp_disposition->x, *dev_x, disposition->size * sizeof(int), stream[1]);
  CUDA_CHECK(res, "cuMemcpyDtoHAsync(tmp_disposition->x, *dev_x): %zd bytes", disposition->size * sizeof(int));
  res = cuMemcpyDtoHAsync(tmp_disposition->y, *dev_y, disposition->size * sizeof(int), stream[2]);
  CUDA_CHECK(res, "cuMemcpyDtoHAsync(tmp_disposition->y, *dev_y): %zd bytes", disposition->size * sizeof(int));

  res = cuStreamSynchronize(stream[0]);
  CUDA_CHECK(res, "cuStreamSynchronize(stream[0])");

  memcpy(disposition->score, tmp_disposition->score, disposition->size * sizeof(float));
...
/*
 ...
 // none
 */
static void calculationScore(int numLevels, int n, const CvLSVMFeaturePyramid *H, const CvLSVMFilterObject **all_F,
                             int *res, CvLSVMFeatureMap *map[], CUdeviceptr **dev_score)
{
  int i, j, k;
  ...
  for (i = 0; i < n + 1; i++)
  {
    memcpy(&(filters[filterIdx[i]]), all_F[i]->H, filterSize[i] * sizeof(float));
  }
  ...
  cu_res = cuTexRefSetAddress(NULL, image_texFi, dev_filter, totalFilterSize * sizeof(float));
  ...
  for (k = Lambda; k < H->numLevels; k++)
  {
    ...
    cu_res = cuMemcpyHtoDAsync(dev_map[0][j], H->pyramid[k]->map, size[0][j] * sizeof(float), convStreams[j]);
    CUDA_CHECK(cu_res, "cuMemcpyHtoDAsync(dev_map[0][%d], H->pyramid[%d]->map): %zd bytes", j, k,
  ...
  for (k = Lambda; k < H->numLevels; k++)
  {
    ...
    cu_res = cuTexRefSetAddress(NULL, image_texMap, dev_map[0][j], size[0][j] * sizeof(float));
    CUDA_CHECK(cu_res, "cuTexRefSetAddress(image_texMap)");
    res[j] = convolutionGPU(all_F[0], H->pyramid[k], &dev_filter, &dev_map[0][j], &dev_filterIdxTbl[j][0],
                            &dev_score[j][0], filterIdx[0], convStreams[0]);
  }
  ...
  for (k = Lambda; k < H->numLevels; k++)
  {
    ...
    cu_res = cuMemcpyHtoDAsync(dev_map[1][j], map[j]->map, size[1][j] * sizeof(float), convStreams[numLevels + 2]);
    CUDA_CHECK(cu_res, "cuMemcpyHtoDAsync(dev_map[1][%d], map[%d]->map): %zd bytes", j, j, size[1][j] * sizeof(float));
    cu_res = cuStreamSynchronize(convStreams[numLevels + 2]);
    CUDA_CHECK(cu_res, "cuStreamSynchronize(convStreams[%d])", numLevels + 2);
    cu_res = cuTexRefSetAddress(NULL, image_texMap, dev_map[1][j], size[1][j] * sizeof(float));
    CUDA_CHECK(cu_res, "cuTexRefSetAddress(image_texMap)");
    ...
    for (i = 0; i < n; i++)
    {
      ...
      {
        res[j] = convolutionGPU(all_F[i + 1], map[j], &dev_filter, &dev_map[1][j], &dev_filterIdxTbl[j][i + 1],
                                &dev_score[j][i + 1], filterIdx[i + 1], convStreams[i + 1]);
      }
    }
  }
...
/*
 ...
 // none
 */
static void distanceTransform(int numLevels, int n, int max_size, const CvLSVMFilterObject **all_F,
                              CvLSVMFeatureMap *map[], CvLSVMFilterDisposition **disposition, CUdeviceptr **dev_score)
{
  ...
  for (j = 0; j < n * numLevels; j++)
  {
    ...
    DistanceTransformTwoDimensionalProblemGPU(all_F[i + 1], map[k], &dev_score[k][i + 1], &dev_distTransWork[l],
    ...
    if (l == DISTANCE_TRANSFORM_STREAMS - 1 || j == n * numLevels - 1)
    {
      ...
      for (int m = 0; m <= l; m++)
      {
        ...
        k = (j - l + m) / n;
        i = (j - l + m) % n;
        dispositionCpyDtoH(&dev_distTransScore[m], &dev_x[m], &dev_y[m], &disposition[k][i], &tmp_disposition,
                           ditTransStreams);
      }
    }
  }
...
/*
 ...
 // Error status
 */
int thresholdFunctionalScore(const CvLSVMFilterObject **all_F, int n, const CvLSVMFeaturePyramid *H, float b,
                             CvLSVMFeatureMap *map[], float scoreThreshold, float **score, CvPoint **points,
                             int **levels, int *kPoints, CvPoint ***partsDisplacement)
{
  ...
  disposition = (CvLSVMFilterDisposition **)malloc(sizeof(CvLSVMFilterDisposition *) * (numLevels));

  for (i = 0; i < numLevels; i++)
  {
    disposition[i] = (CvLSVMFilterDisposition *)malloc(sizeof(CvLSVMFilterDisposition) * n);
  }
  ...
  for (k = 0; k < numLevels; k++)
  {
    for (i = 0; i < n; i++)
    {
      dispositionMalloc(all_F[i + 1], map[k], &disposition[k][i], &max_size);
    }
  }
  distanceTransform(numLevels, n, max_size, all_F, map, disposition, dev_score);
  ...
  for (l = Lambda; l < H->numLevels; l++)
  {
    ...
    {
      res[k] = sumScore(all_F, n, H, l, b, scoreThreshold, &disposition[k][0], &dev_score[k][0], map[k], &(tmpScore[k]),
                        tmpPoints[k], &(tmpKPoints[k]), tmpPartsDisplacement[k]);
  ...
  all_tm.stop();
  cout << "}" << endl
       << "(matching.cpp)thresholdFunctionalScore END time = " << all_tm.getTimeSec() << " sec" << endl;
#endif
...
/*
 ...
 // Error status
 */
int nonMaximumSuppression(int numBoxes, const CvPoint *points, const CvPoint *oppositePoints, const float *score,
                          float overlapThreshold, int *numBoxesOut, CvPoint **pointsOut, CvPoint **oppositePointsOut,
                          float **scoreOut)
{
  int i, j, index;
