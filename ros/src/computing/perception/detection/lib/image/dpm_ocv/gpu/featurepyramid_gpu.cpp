...
// Error status
*/
int calculateHistogramGPULaunch(const int k, CvLSVMFeatureMapGPU *dev_img, CvLSVMFeatureMapGPU *dev_r,
                                CvLSVMFeatureMapGPU *dev_alfa, CUstream stream)
{
  ...
  void *calc_hist_kernel_arg[] = {(void *)&dev_img->map, (void *)&dev_r->map, (void *)&dev_alfa->map, (void *)&width,
                                  (void *)&height,       (void *)&width_step, (void *)&numChannels};

  thread_num_x = (width < std::sqrt(max_threads_num)) ? width : std::sqrt(max_threads_num);
  thread_num_y = (height < std::sqrt(max_threads_num)) ? height : std::sqrt(max_threads_num);
  thread_num_z = 1;
  ...
  res = cuLaunchKernel(calculateHistogram_func[0], block_num_x, block_num_y, block_num_z, thread_num_x, thread_num_y,
                       thread_num_z, sharedMemBytes, stream, calc_hist_kernel_arg, NULL);
...
// Error status
*/
int getFeatureMapsGPULaunch(const int k, CvLSVMFeatureMapGPU *dev_r, CvLSVMFeatureMapGPU *dev_alfa,
                            CUdeviceptr *dev_nearest, CUdeviceptr *dev_w, CvLSVMFeatureMapGPU *dev_map, CUstream stream)
{
  int sizeX, sizeY;
  ...
  void *get_feature_maps_kernel_arg[] = {(void *)&dev_r->map, (void *)&dev_alfa->map, (void *)dev_nearest,
                                         (void *)dev_w, (void *)&dev_map->map, (void *)&width, (void *)&height,
                                         (void *)&k, (void *)&p};

  thread_num_x = (sizeX < std::sqrt(max_threads_num)) ? sizeX : std::sqrt(max_threads_num);
  thread_num_y = (sizeY < std::sqrt(max_threads_num)) ? sizeY : std::sqrt(max_threads_num);
  thread_num_z = 1;
  ...
  res = cuLaunchKernel(getFeatureMaps_func[0], block_num_x, block_num_y, block_num_z, thread_num_x, thread_num_y,
                       thread_num_z, sharedMemBytes, stream, get_feature_maps_kernel_arg, NULL);
...
// Error status
*/
int calculateNormGPULaunch(CvLSVMFeatureMapGPU *dev_map_in, CvLSVMFeatureMapGPU *dev_norm, CUstream stream)
{
  int sizeX, sizeY, xp;
  int thread_num_x, thread_num_y, thread_num_z;
  int block_num_x, block_num_y, block_num_z;
  ...
  void *calc_norm_kernel_arg[] = {
      (void *)&dev_map_in->map, (void *)&dev_norm->map, (void *)&sizeX, (void *)&sizeY, (void *)&xp,
  };
  ...
  res = cuLaunchKernel(calculateNorm_func[0], block_num_x, block_num_y, block_num_z, thread_num_x, thread_num_y,
                       thread_num_z, sharedMemBytes, stream, calc_norm_kernel_arg, NULL);
...
// Error status
*/
int normalizeGPULaunch(const float alfa, CvLSVMFeatureMapGPU *dev_map_in, CvLSVMFeatureMapGPU *dev_norm,
                       CvLSVMFeatureMapGPU *dev_map_out, CUstream stream)
{
  int sizeX, sizeY;
  int thread_num_x, thread_num_y, thread_num_z;
  int block_num_x, block_num_y, block_num_z;
  ...
  void *normalize_kernel_arg[] = {
      (void *)&dev_map_in->map, (void *)&dev_norm->map, (void *)&dev_map_out->map, (void *)&sizeX, (void *)&sizeY,
      (void *)&alfa,
  };
  ...
  res = cuLaunchKernel(normalizeAndTruncate_func[0], block_num_x, block_num_y, block_num_z, thread_num_x, thread_num_y,
                       thread_num_z, sharedMemBytes, stream, normalize_kernel_arg, NULL);
...
// Error status
*/
int PCAFeatureMapsAddNullableBorderGPULaunch(CvLSVMFeatureMapGPU *dev_map_in, CvLSVMFeatureMapGPU *dev_map_out,
                                             const int bx, const int by, CUstream stream)
{
  int sizeX, sizeY, p;
  int thread_num_x, thread_num_y, thread_num_z;
  int block_num_x, block_num_y, block_num_z;
  ...
  void *pca_kernel_arg[] = {(void *)&dev_map_in->map, (void *)&dev_map_out->map, (void *)&sizeX, (void *)&sizeY,
                            (void *)&p, (void *)&bx, (void *)&by};
  ...
  res = cuLaunchKernel(PCAFeatureMapsAddNullableBorder_func[0], block_num_x, block_num_y, block_num_z, thread_num_x,
                       thread_num_y, thread_num_z, sharedMemBytes, stream, pca_kernel_arg, NULL);
...
// Error status
*/
int getFeatureMapsGPUStream(const int numStep, const int k, CvLSVMFeatureMapGPU **devs_img,
                            CvLSVMFeatureMapGPU **devs_map, CUstream *streams)
{
  int sizeX, sizeY;
  int p, px;
  int height, width;
  ...
  devs_r = (CvLSVMFeatureMapGPU **)malloc(sizeof(CvLSVMFeatureMapGPU *) * numStep);
  devs_alfa = (CvLSVMFeatureMapGPU **)malloc(sizeof(CvLSVMFeatureMapGPU *) * numStep);
  ...
  res = cuMemcpyHtoDAsync(dev_nearest, nearest, sizeof(int) * size_nearest, streams[numStep - 1]);
  CUDA_CHECK(res, "cuMemcpyHtoDAsync(dev_nearest, nearest, %zd bytes)", sizeof(int) * size_nearest);
  res = cuMemcpyHtoDAsync(dev_w, w, sizeof(float) * size_w, streams[numStep - 1]);
  ...
  for (i = 0; i < numStep; i++)
  {
    ...
    // launch kernel
    calculateHistogramGPULaunch(k, devs_img[i], devs_r[i], devs_alfa[i], streams[i]);
  }

  for (i = 0; i < numStep; i++)
  {
    getFeatureMapsGPULaunch(k, devs_r[i], devs_alfa[i], &dev_nearest, &dev_w, devs_map[i], streams[i]);
  }
...
// Error status
*/
int normalizeAndTruncateGPUStream(const int numStep, const float alfa, CvLSVMFeatureMapGPU **devs_map_in,
                                  CvLSVMFeatureMapGPU **devs_map_out, CUstream *streams)
{
  ...
  devs_norm = (CvLSVMFeatureMapGPU **)malloc(sizeof(CvLSVMFeatureMapGPU *) * (numStep));
  ...
  for (i = 0; i < numStep; i++)
  {
    ...
    res = cuMemsetD32Async(devs_map_out[i]->map, 0, size_map_out, streams[i]);
  ...
  {
    // launch kernel
    normalizeGPULaunch(alfa, devs_map_in[i], devs_norm[i], devs_map_out[i], streams[i]);
  }
...
// Error status
*/
int PCAFeatureMapsGPUStream(const int numStep, const int bx, const int by, CvLSVMFeatureMapGPU **devs_map_in,
                            CvLSVMFeatureMap **feature_maps, CUstream *streams)
{
  int sizeX, sizeY, pp;
  int size_map_pca;
  int i;
  ...
  devs_map_pca = (CvLSVMFeatureMapGPU **)malloc(sizeof(CvLSVMFeatureMapGPU *) * (numStep));
  ...
  for (i = 0; i < numStep; i++)
  {
    ...
    // initilize device memory value of 0
    res = cuMemsetD32Async(devs_map_pca[i]->map, 0, size_map_pca, streams[i]);
    CUDA_CHECK(res, "cuMemset(dev_map_pca[%d]->map)", i);

    // launch kernel
    PCAFeatureMapsAddNullableBorderGPULaunch(devs_map_in[i], devs_map_pca[i], bx, by, streams[i]);
  }
  ...
  for (i = 0; i < numStep; i++)
  {
    ...
    res = cuMemcpyDtoHAsync(feature_maps[i]->map, devs_map_pca[i]->map, sizeof(float) * size_map_pca, streams[i]);
...
// Error status
*/
static int getPathOfFeaturePyramidGPUStream(IplImage *image, float step, int numStep, int startIndex, int sideLength,
                                            int bx, int by, CvLSVMFeaturePyramid **maps)
{
  ...
  devs_img = (CvLSVMFeatureMapGPU **)malloc(sizeof(CvLSVMFeatureMapGPU *) * (numStep));
  devs_map_pre_norm = (CvLSVMFeatureMapGPU **)malloc(sizeof(CvLSVMFeatureMapGPU *) * (numStep));
  devs_map_pre_pca = (CvLSVMFeatureMapGPU **)malloc(sizeof(CvLSVMFeatureMapGPU *) * (numStep));
  streams = (CUstream *)malloc(sizeof(CUstream) * (numStep));
  feature_maps = (CvLSVMFeatureMap **)malloc(sizeof(CvLSVMFeatureMap *) * (numStep));
  ...
  for (i = 0; i < numStep; i++)
  {
    ...
    allocFeatureMapObjectGPU<float>(&devs_img[i], width, height, numChannels);
    allocFeatureMapObjectGPU<float>(&devs_map_pre_norm[i], sizeX, sizeY, p);
    allocFeatureMapObjectGPU<float>(&devs_map_pre_pca[i], newSizeX, newSizeY, pp);
  ...
  getFeatureMapsGPUStream(numStep, sideLength, devs_img, devs_map_pre_norm, streams);

  normalizeAndTruncateGPUStream(numStep, Val_Of_Truncate, devs_map_pre_norm, devs_map_pre_pca, streams);

  PCAFeatureMapsGPUStream(numStep, bx, by, devs_map_pre_pca, feature_maps, streams);
...
int getFeaturePyramid(IplImage *image, CvLSVMFeaturePyramid **maps, const int bx, const int by)
{
  IplImage *imgResize;
  float step;
  ...
  {
    imgResize = cvCreateImage(cvSize(image->width, image->height), IPL_DEPTH_32F, 3);
  ...
  getPathOfFeaturePyramidGPUStream(imgResize, step, Lambda, 0, Side_Length / 2, bx, by, maps);

  getPathOfFeaturePyramidGPUStream(imgResize, step, numStep, Lambda, Side_Length, bx, by, maps);
