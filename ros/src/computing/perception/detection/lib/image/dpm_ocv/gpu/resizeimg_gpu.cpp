...
void uploadImageToGPU1D(IplImage *img)
{
  ...
  res = cuTexRefSetAddress(NULL, image_texref, dev_image, sizeof(float) * size_img);
...
void resizeLaunchUsingTex(IplImage *img, float scale, size_t type, CvLSVMFeatureMapGPU *dev_img, CUstream stream)
{
  int W, H, tW, tH;
  int nChannels;
  ...
  void *kernel_arg[] = {(void *)&dev_img->map, (void *)&W, (void *)&H, (void *)&tW, (void *)&tH, (void *)&nChannels,
                        (void *)&widthStepIn, (void *)&widthStepOut};
  ...
  res = cuLaunchKernel(BilinearKernelTex32F_func[0], block_num_x, block_num_y, block_num_z, thread_num_x, thread_num_y,
                       thread_num_z, sharedMemBytes, stream, kernel_arg, NULL);
  CUDA_CHECK(res, "cuLaunchKernel(BilinearKernelTex32F)");
}

void resizeGPUStream(const int numStep, IplImage *img, float *scale, CvLSVMFeatureMapGPU **devs_img, CUstream *stream)
{
  for (int i = 0; i < numStep; i++)
  {
    resizeLaunchUsingTex(img, scale[i], sizeof(float), devs_img[i], stream[i]);
  }
}
