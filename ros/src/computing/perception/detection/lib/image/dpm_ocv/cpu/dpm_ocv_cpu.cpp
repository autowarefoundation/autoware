...
extern int searchObjectThresholdSomeComponents(const CvLSVMFeaturePyramid *H, const CvLSVMFilterObject **filters,
          ...
          if ((i * k + ii > 0) && (i * k + ii < height - 1) && (j * k + jj > 0) && (j * k + jj < width - 1))
          {
                ...
                (j + nearest[jj] <= sizeX - 1))
            {
              ...
              (*map)->map[(i + nearest[ii]) * stringSize + (j + nearest[jj]) * (*map)->numFeatures + alfa[d * 2 + 1] +
                          num_bins] += r[d] * w[ii * 2 + 1] * w[jj * 2 + 1];
            }
          }
        } /*for(jj = 0; jj < k; jj++)*/
      }   /*for(ii = 0; ii < k; ii++)*/
...
static int getPathOfFeaturePyramid(IplImage *image, float step, int numStep, int startIndex, int sideLength,
                                   CvLSVMFeaturePyramid **maps, int num_bins)
{
...
static int getFeaturePyramid(IplImage *image, CvLSVMFeaturePyramid **maps, int lambda, int num_cells, int num_bins)
{
  IplImage *imgResize;
  float step;
  int numStep;
  ...
  getPathOfFeaturePyramid(imgResize, step, lambda, 0, num_cells / 2, maps, num_bins);
  getPathOfFeaturePyramid(imgResize, step, numStep, lambda, num_cells, maps, num_bins);
  if (image->depth != IPL_DEPTH_32F)
  {
...
static CvLSVMFeaturePyramid *createFeaturePyramidWithBorder(IplImage *image, int maxXBorder, int maxYBorder, int lambda,
                                                            int num_cells, int num_bins)
{
  int opResult;
  int bx, by;
  int level;
...
static CvSeq *cvLatentSvmDetectObjects(IplImage *image, CvLatentSvmDetector *detector, CvMemStorage *storage,
                                       float overlap_threshold, int numThreads, double score_threshold, int lambda,
                                       int num_cells, int num_bins)
{
  ...
  error = searchObjectThresholdSomeComponents(H, (const CvLSVMFilterObject **)(detector->filters),
                                              ...
                                              score_threshold, &points, &oppPoints, &score, &kPoints, numThreads);
...
void DPMOCVCPULatentSvmDetector::detect(const cv::Mat &image, std::vector<ObjectDetection> &objectDetections,
                                        float overlapThreshold, int numThreads, double score_threshold, int lambda,
                                        int num_cells, int num_bins)
{
  ...
  for (size_t classID = 0; classID < detectors.size(); classID++)
  {
    ...
    CvSeq *detections = cvLatentSvmDetectObjects(&image_ipl, detectors[classID], storage, overlapThreshold, numThreads,
                                                 score_threshold, lambda, num_cells, num_bins);
