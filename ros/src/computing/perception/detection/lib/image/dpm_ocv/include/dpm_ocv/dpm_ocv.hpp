...
public:
  ...
  void detect(const cv::Mat& image, std::vector<ObjectDetection>& objectDetections, float overlapThreshold,
              int numThreads, double score_threshold, int lambda, int num_cells, int num_bins);
...
public:
  explicit DPMOCVGPULatentSvmDetector(const std::vector<std::string>& filenames, const float scoreThreshold);
  ~DPMOCVGPULatentSvmDetector();

  bool load(const std::vector<std::string>& filenames, const float scoreThreshold);
  void detect(const cv::Mat& image, std::vector<ObjectDetection>& objectDetections, float overlapThreshold,
              int numThreads, const unsigned int lambda, const unsigned int sideLength, const float valOfTruncate);
