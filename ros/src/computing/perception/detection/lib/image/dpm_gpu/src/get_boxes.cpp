...
//detect boundary box
FLOAT *get_boxes(FLOAT **features,FLOAT *scales,int *feature_size, MODEL *MO,
		...
		for(int level=interval; level<L_MAX; level++){
			...
			for(int j=0; j<NoC; j++) {
				for(int k=0; k<numpart[j]; k++) {
					...
					add_part_calculation(SCORE_array[L][j], M_array[L][j][k], R_S, PSSIZE, ax_array[L][j][k], ay_array[L][j][k]);
				}
			}
		}
