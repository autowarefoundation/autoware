...
static FLOAT *partbox(int x, int y, int ax, int ay, FLOAT scale, int padx, int pady, int *psize, int *lx, int *ly,
                      int *ssize)
{
  FLOAT *Out = (FLOAT *)malloc(sizeof(FLOAT) * 4);
  int probex = (x - 1) * 2 + ax;
...
static void calc_a_score_GPU(FLOAT *ac_score, FLOAT **score, int *ssize_start, Model_info *MI, FLOAT scale,
                             int *size_score_array, int NoC)
{
  CUresult res;
  ...
  void *kernel_args[] = {(void *)&IWID,   (void *)&IHEI, (void *)&scale, (void *)&padx_n,
                         (void *)&pady_n, &RX_dev,       &RY_dev,        &ac_score_dev,
                         &score_dev,      &ssize_dev,    (void *)&NoC,   &size_score_dev};
...
// detect boundary box
FLOAT *dpm_ttic_gpu_get_boxes(FLOAT **features, FLOAT *scales, int *feature_size, GPUModel *MO, int *detected_count,
                              FLOAT *acc_score, FLOAT thresh)
{
  // constant parameters
  ...
  rootmatch = fconvsMT_GPU(featp2, SUM_SIZE_feat, rootfilter, rootsym, 1, NoR, new_PADsize, RF_size, rm_size_array,
                           L_MAX, interval, feature_size, padx, pady, MO->MI->max_X, MO->MI->max_Y, ROOT);
  ...
  if (NoP > 0)
  {
    ...
    partmatch = fconvsMT_GPU(featp2, SUM_SIZE_feat, partfilter, part_sym, 1, NoP, new_PADsize, part_size, pm_size_array,
                             L_MAX, interval, feature_size, padx, pady, MO->MI->max_X, MO->MI->max_Y, PART);
    ...
    for (int level = interval; level < L_MAX; level++)
    {
      ...
      for (int j = 0; j < NoC; j++)
      {
        for (int k = 0; k < numpart[j]; k++)
        {
          ...
          dpm_ttic_add_part_calculation(SCORE_array[L][j], M_array[L][j][k], R_S, PSSIZE, ax_array[L][j][k],
                                        ay_array[L][j][k]);
        }
      }
    }
    ...
    for (int j = 0; j < NoC; j++)
    {
      ...
      for (int k = 0; k < GMN; k++)
      {
        ...
        for (int pp = 0; pp < numpart[j]; pp++)
        {
          ...
          FLOAT *PB = partbox(x, y, ax_array[L][j][pp], ay_array[L][j][pp], scale, padx, pady, PBSIZE,
                              Ix_array[L][j][pp], Iy_array[L][j][pp], Isize);
