...
/*********************************************************************/
FLOAT *dpm_ttic_gpu_Ipl_to_FLOAT_forGPU(IplImage *Input)  // get intensity data (FLOAT) of input
{
...
{
  CUresult res;
  res = cuMemHostAlloc((void **)&image_idx_incrementer, LEN * sizeof(int), CU_MEMHOSTALLOC_PORTABLE);
...
static void *bilinear_resizing(void *arg)
{
  ...
  void *kernel_arg[] = {(void *)&src_height, (void *)&src_width, &resized_image_dev, (void *)&dst_height,
                        (void *)&dst_width,  (void *)&hfactor,   (void *)&wfactor,   (void *)&level};
...
void dpm_ttic_gpu_resize_byGPU(FLOAT *org_image, int *org_image_size, int *resized_image_size, int interval, int LEN,
                               CUstream *stream_array)
{
  ...
  for (int level = interval; level < 2 * interval; level++)
  {
    ...
    unsigned long long int src_ptr = (unsigned long long int)resized_image_dev +
                                     (unsigned long long int)(image_idx_incrementer[level - interval] * sizeof(FLOAT));
    unsigned long long int dst_ptr = (unsigned long long int)resized_image_dev +
                                     (unsigned long long int)(image_idx_incrementer[level] * sizeof(FLOAT));
    int copy_size = resized_image_size[level * 3] * resized_image_size[level * 3 + 1] *
                    resized_image_size[level * 3 + 2] * sizeof(FLOAT);
    res = cuMemcpyDtoDAsync((CUdeviceptr)dst_ptr, (CUdeviceptr)src_ptr, copy_size, stream_array[level]);
  }
...
void dpm_ttic_gpu_calc_resized_image_size(int *org_image_size, int *resized_image_size, int interval, FLOAT sc,
                                          int max_scale, FLOAT *scale_array)
{
