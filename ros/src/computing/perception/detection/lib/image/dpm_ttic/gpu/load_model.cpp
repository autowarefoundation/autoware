...
static Model_info *load_modelinfo(const char *filename)
{
  ...
  if (res != CUDA_SUCCESS)
  {
    std::cerr << "Failed: cuMemHostAlloc(MI->def) : " << cuda_response_to_string(res) << std::endl;
    std::exit(1);
  }
...
static Rootfilters *load_rootfilter(const char *filename)
{
  ...
  for (int i = 0; i < RF->NoR; i++)
  {
    ...
    if (res != CUDA_SUCCESS)
    {
      std::cerr << "Failed: cuMemHostAlloc(RF->rootfilter): " << cuda_response_to_string(res) << std::endl;
      std::exit(1);
    }
#else
  ...
  if (res != CUDA_SUCCESS)
  {
    std::cerr << "Failed: cuMemHostAlloc(dst_root): " << cuda_response_to_string(res) << std::endl;
    std::exit(1);
  }
...
static Partfilters *load_partfilter(const char *filename)
{
  ...
  for (int i = 0; i < PF->NoP; i++)
  {
    ...
    if (res != CUDA_SUCCESS)
    {
      std::cerr << "Failed: cuMemHostAlloc(PF->partfilter): " << cuda_response_to_string(res) << std::endl;
      std::exit(1);
    }
#else
  ...
  if (res != CUDA_SUCCESS)
  {
    std::cerr << "Failed: cuMemHostAlloc(dst_part): " << cuda_response_to_string(res) << std::endl;
    std::exit(1);
  }
...
void dpm_ttic_gpu_free_model(GPUModel *MO)
{
  ...
  if (res != CUDA_SUCCESS)
  {
    std::cerr << "Failed: cuMemFreeHost(MO->MI->def): " << cuda_response_to_string(res) << std::endl;
    std::exit(1);
  }
  ...
  for (int i = 0; i < MO->RF->NoR; i++)
  {
    ...
    if (res != CUDA_SUCCESS)
    {
      std::cerr << "Failed: cuMemFreeHost(MO->RF->rootfilter): " << cuda_response_to_string(res) << std::endl;
      std::exit(1);
    }
#endif
#endif
  ...
  if (res != CUDA_SUCCESS)
  {
    std::cerr << "Failed: cuMemFreeHost(MO->RF->rootfilter[0]): " << cuda_response_to_string(res) << std::endl;
    std::exit(1);
  }
#endif
#endif
  ...
  for (int i = 0; i < MO->PF->NoP; i++)
  {
    ...
    if (res != CUDA_SUCCESS)
    {
      std::cerr << "Failed: cuMemFreeHost(MO->PF->partfilter): " << cuda_response_to_string(res) << std::endl;
      std::exit(1);
    }
#endif
#endif
  ...
  if (res != CUDA_SUCCESS)
  {
    std::cerr << "Failed: cuMemFreeHost(MO->PF->partfilter[0]): " << cuda_response_to_string(res) << std::endl;
    std::exit(1);
  }
#endif
#endif
