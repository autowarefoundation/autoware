...
#ifdef ORIGINAL
static FLOAT *calc_feature(FLOAT *SRC,   // resized image
                           ...
                           )
{
  ...
  const int blocks[2] = {(int)floor(double(height) / double(sbin) + 0.5),
...
static void calc_feature_byGPU(
    ...
    int *FTSIZE,  // size of feature(output)
    int sbin,     // block size desicion element for each filter
    int level, CUdeviceptr hist_dev, CUdeviceptr norm_dev, CUdeviceptr feat_dev, CUstream stream)
{
  /* rename argument */
  ...
  void *kernel_args_hist[] = {&hist_dev, (void *)&sbin, (void *)&visible[0], (void *)&visible[1], (void *)&level};
  ...
  int thread_num_x = (visible[1] - 1 < sqrt(max_thread_num)) ? visible[1] - 1 : sqrt(max_thread_num);
  int thread_num_y = (visible[0] - 1 < sqrt(max_thread_num)) ? visible[0] - 1 : sqrt(max_thread_num);
...
// MY_CUDA_CHECK(res, "cuStreamSynchronize(stream)");
  ...
  /* compute energy in each block by summing over orientations */
  void *kernel_args_norm[] = {&hist_dev, &norm_dev, (void *)&blocks[0], (void *)&blocks[1], (void *)&level};

  /* decide CUDA block shape */
  thread_num_x = (blocks[1] < sqrt(max_thread_num)) ? blocks[1] : sqrt(max_thread_num);
  thread_num_y = (blocks[0] < sqrt(max_thread_num)) ? blocks[0] : sqrt(max_thread_num);
...
// MY_CUDA_CHECK(res, "cuStreamSynchronize(stream)");
  ...
  /* compute featuers */
  void *kernel_args_feat[] = {&hist_dev,       &norm_dev,          &feat_dev,          (void *)&out[0],
                              (void *)&out[1], (void *)&blocks[0], (void *)&blocks[1], (void *)&level};

  /* decide CUDA block shape */
  thread_num_x = (out[1] < sqrt(max_thread_num)) ? out[1] : sqrt(max_thread_num);
  thread_num_y = (out[0] < sqrt(max_thread_num)) ? out[0] : sqrt(max_thread_num);
...
#else
static void ini_thread_data_fouGPU(thread_data_forGPU *TD, int *INSIZE, int sbin, int level, CUdeviceptr hist_dev,
                                   CUdeviceptr norm_dev, CUdeviceptr feat_dev, CUstream stream)
{
  //  TD->IM       = IM;
  memcpy(TD->ISIZE, INSIZE, sizeof(int) * 3);
  TD->FSIZE[0] = 0;
...
FLOAT **gpu_calc_f_pyramid(IplImage *Image, Model_info *MI, int *FTSIZE, FLOAT *scale)  // calculate feature pyramid
{
  /* constant parameters */
...
/* thread for feature calculation */
  ...
  dpm_ttic_gpu_calc_resized_image_size(org_image_size, resized_image_size, interval, sc, max_scale, scale);
  ...
  dpm_ttic_gpu_resize_byGPU(org_image, org_image_size, resized_image_size, interval, LEN, stream);
...
/*****************************************************************/
  ...
  res = cuMemHostAlloc((void **)&hist_ptr_incrementer, LEN * sizeof(unsigned long long int), CU_MEMHOSTALLOC_PORTABLE);
  MY_CUDA_CHECK(res, "cuMemHostAlloc(hist_ptr_incrementer)");

  res = cuMemHostAlloc((void **)&norm_ptr_incrementer, LEN * sizeof(unsigned long long int), CU_MEMHOSTALLOC_PORTABLE);
  MY_CUDA_CHECK(res, "cuMemHostAlloc(norm_ptr_incrementer)");

  res = cuMemHostAlloc((void **)&feat_ptr_incrementer, LEN * sizeof(unsigned long long int), CU_MEMHOSTALLOC_PORTABLE);
  ...
  for (int level = 0; level < LEN; level++)
  {
    ...
    int blocks_inner[2] = {(int)floor((double)height_inner / (double)sbin_inner + 0.5),
                           (int)floor((double)width_inner / (double)sbin_inner + 0.5)};

    /* size of Output features size */
    int out_size[3] = {max_i(blocks_inner[0] - 2, 0), max_i(blocks_inner[1] - 2, 0), 27 + 4};
  ...
  res = cuTexRefSetAddress(NULL, hist_ptr_incrementer_texref, hist_ptr_incrementer_dev,
                           LEN * sizeof(unsigned long long int));
  MY_CUDA_CHECK(res, "cuTexRefSetAddress(hist_ptr_incrementer_dev)");

  res = cuTexRefSetAddress(NULL, norm_ptr_incrementer_texref, norm_ptr_incrementer_dev,
                           LEN * sizeof(unsigned long long int));
  MY_CUDA_CHECK(res, "cuTexRefSetAddress(norm_ptr_incrementer_dev)");

  res = cuTexRefSetAddress(NULL, feat_ptr_incrementer_texref, feat_ptr_incrementer_dev,
                           LEN * sizeof(unsigned long long int));
...
/* "first" 2x interval */
#ifdef ORIGINAL
    ini_thread_data(&td[t_count], resized_image[ii], resized_image_size + ii * 3, sbin2, ii);  // initialize thread
#else
    ini_thread_data_fouGPU(&td[t_count],
                           //                             resized_image[ii],
                           resized_image_size + ii * 3, sbin2, ii, hist_dev, norm_dev, feat_dev,
...
/* "second" 1x interval */
#ifdef ORIGINAL
    ini_thread_data(&td[t_count], resized_image[ii + interval], resized_image_size + ii * 3, sbin,
                    ii + interval);  // initialize thread
#else
    ini_thread_data_fouGPU(&td[t_count],
                           //                             resized_image[ii+interval],
                           resized_image_size + ii * 3, sbin, ii + interval, hist_dev, norm_dev, feat_dev,
    ...
    {
#ifdef ORIGINAL
      ini_thread_data(&td[t_count], resized_image[jj + interval], resized_image_size + (jj + interval) * 3, sbin,
                      jj + interval);  // initialize thread
#else
      ini_thread_data_fouGPU(&td[t_count],
                             //                                 resized_image[jj+interval],
                             resized_image_size + (jj + interval) * 3, sbin, jj + interval, hist_dev, norm_dev,
                             feat_dev,
