...
class RcnnDetector
{
  ...
  */
  std::vector<cv::Scalar> GenerateProposals(unsigned int width, unsigned int height, unsigned int slices = 16,
  ...
  */
  std::vector<RectClassScore<float>> GetRectClassesScored(std::vector<cv::Scalar>& in_proposals,
                                                          ...
                                                          const float* in_probabilities, unsigned int width,
                                                          unsigned int height);

  /**
   * @brief Apply non maximum suppresion to the detections
   */
  std::vector<RectClassScore<float>> ApplyNonMaximumSuppresion(std::vector<RectClassScore<float>> in_source,
...
public:
  /**
  * @brief FastRCNN Constructor, creates the Caffe network,  loads the definition(.proto) and pretrained
  * model(.caffemodel), also sets GPU mode and  Device ID
  */
  RcnnDetector(std::string& network_definition_file, std::string& pre_trained_model_file, bool use_gpu = true,
               unsigned int gpu_id = 0);

  /**
  * @brief Starts detection of the 21 classes on an image, using the parameters
  */
  std::vector<RectClassScore<float>> Detect(cv::Mat& in_image, const std::vector<unsigned int>& in_classes,
                                            float in_threshold = 0.7, unsigned int in_slices = 16,
                                            float in_box_overlap = 0.7, float in_nms_threshold = 0.3);
  ...
  */
  inline void SetPixelMean(cv::Scalar new_pixel_mean)
  {
    pixel_mean_ = new_pixel_mean;
  }
};
