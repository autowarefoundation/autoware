...
int main(int argc, char **argv) {
    ...
    Synchronizer<autoware_detection_msgs::ImageObj, autoware_sensing_msgs::PointsImage, autoware_detection_msgs::ImageObjRanged> synchronizer(sub1, sub2, pub1, pub2, req, ns);
    synchronizer.run();
...
/* user var */
boost::circular_buffer<autoware_detection_msgs::ImageObj> image_obj_ringbuf(10);
boost::circular_buffer<autoware_sensing_msgs::PointsImage> vscan_image_ringbuf(10);
...
#if _REQ_PUB
autoware_detection_msgs::ImageObj* p_image_obj_buf;
autoware_sensing_msgs::PointsImage* p_vscan_image_buf;

void publish_msg(autoware_detection_msgs::ImageObj* p_image_obj_buf, autoware_sensing_msgs::PointsImage* p_vscan_image_buf) {
    ROS_INFO("publish");
...
bool publish() {
    if (buf_flag) {
        ...
        if (get_time(&(image_obj_ringbuf.front().header)) >= get_time(&(vscan_image_ringbuf.front().header))) {
            ...
            boost::circular_buffer<autoware_detection_msgs::ImageObj>::iterator it = image_obj_ringbuf.begin();
        ...
        // image_obj < vscan_image
            ...
            boost::circular_buffer<autoware_sensing_msgs::PointsImage>::iterator it = vscan_image_ringbuf.begin();
...
int main(int argc, char **argv) {
    ...
    image_obj__pub = nh.advertise<autoware_detection_msgs::ImageObj>("/sync_ranging/image_obj", 5);
    vscan_image__pub = nh.advertise<autoware_sensing_msgs::PointsImage>("/sync_ranging/vscan_image", 5);
...
#if 0
autoware_detection_msgs::ImageObj image_obj_buf;
autoware_sensing_msgs::PointsImage vscan_image_buf;
