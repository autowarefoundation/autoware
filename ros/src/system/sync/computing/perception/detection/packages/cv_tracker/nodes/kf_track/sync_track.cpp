...
int main(int argc, char **argv) {
    ...
    Synchronizer<autoware_detection_msgs::ImageObjRange, sensor_msgs::Image, autoware_detection_msgs::ImageObjTracked> synchronizer(sub1, sub2, pub1, pub2, req, ns);
    synchronizer.run();
...
/* user var */
boost::circular_buffer<autoware_detection_msgs::ImageObjRanged> image_obj_ranged_ringbuf(10);
...
#if _REQ_PUB
autoware_detection_msgs::ImageObjRange* p_image_obj_ranged_buf;
sensor_msgs::Image* p_image_raw_buf;

void publish_msg(autoware_detection_msgs::ImageObjRange* p_image_obj_ranged_buf, sensor_msgs::Image* p_image_raw_buf) {
    ROS_INFO("publish");
...
bool publish() {
    if (buf_flag) {
        ...
        if (get_time(&(image_obj_ranged_ringbuf.front().header)) >= get_time(&(image_raw_ringbuf.front().header))) {
            ...
            boost::circular_buffer<autoware_detection_msgs::ImageObjRanged>::iterator it = image_obj_ranged_ringbuf.begin();
...
int main(int argc, char **argv) {
    ...
    image_obj_ranged__pub = nh.advertise<autoware_detection_msgs::ImageObjRanged>("/sync_tracking/image_obj_ranged", 5);
...
#if 0
autoware_detection_msgs::ImageObjRanged image_obj_ranged_buf;
