...
int main(int argc, char **argv) {
    ...
    Synchronizer<autoware_detection_msgs::ImageObjTracked, geometry_msgs::PoseStamped, autoware_detection_msgs::ObjLabel> synchronizer(sub1, sub2, pub1, pub2, req, ns);
    synchronizer.run();
...
/* user var */
boost::circular_buffer<autoware_detection_msgs::ImageObjTracked> image_obj_tracked_ringbuf(10);
...
#if _REQ_PUB
autoware_detection_msgs::ImageObjTracked* p_image_obj_tracked_buf;
geometry_msgs::PoseStamped* p_current_pose_buf;

void publish_msg(autoware_detection_msgs::ImageObjTracked* p_image_obj_tracked_buf, geometry_msgs::PoseStamped* p_current_pose_buf) {
    ROS_INFO("publish");
...
bool publish() {
    if (buf_flag) {
        ...
        if (get_time(&(image_obj_tracked_ringbuf.front().header)) >= get_time(&(current_pose_ringbuf.front().header))) {
            ...
            boost::circular_buffer<autoware_detection_msgs::ImageObjTracked>::iterator it = image_obj_tracked_ringbuf.begin();
...
int main(int argc, char **argv) {
    ...
    image_obj_tracked__pub = nh.advertise<autoware_detection_msgs::ImageObjTracked>("/sync_reprojection/image_obj_tracked", 5);
...
#if 0
autoware_detection_msgs::ImageObjTracked image_obj_tracked_buf;
