...
private:
    ...
    std::string generateText(std::vector<autoware_system_msgs::DiagnosticStatus> status);
    jsk_rviz_plugins::OverlayText generateOverlayText(autoware_system_msgs::SystemStatus status,uint8_t level);
    std::vector<autoware_system_msgs::DiagnosticStatus> filterNodeStatus(autoware_system_msgs::SystemStatus status,uint8_t level);
