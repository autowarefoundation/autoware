^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package fastvirtualscan
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
1.0.0 (2016-12-22)
------------------
* Make ranges 0 when there are near objects
* Add missing dependencies
* Remove needless compiling flags
* Update vscan distance value for free space
* Clean up Qt5 configuration
  Use pkg-config as possible instead of absolute pathes.
* Use c++11 option instead of c++0x
  We can use newer compilers which support 'c++11' option
* Initial commit for public release
* Contributors: HMW-Alexander, Shinpei Kato, Syohei YOSHIDA, TomohitoAndo
