...
namespace autoware_rviz_plugins
{
  OverlayGpsDisplay::OverlayGpsDisplay()
  {
    ...
    map_image_path_ = ros::package::getPath("autoware_rviz_plugins") + "/media/map.png";
  ...
  void OverlayGpsDisplay::processMessage(const sensor_msgs::NavSatFix::ConstPtr& msg)
  {
    if(msg->header.seq%messages_per_plot_property_->getInt()==0)
    {
      ...
      {
        this->setStatus(rviz::StatusProperty::Level::Error, "InvalidRequestURL", "request url is invalid. Check API Key.");
        return;
      }
      this->setStatus(rviz::StatusProperty::Level::Ok, "InvalidRequestURL", "request url is valid.");
      if(!overlay_)
      {
        static int count = 0;
  ...
  {
    std::string map_downloader_path = ros::package::getPath("autoware_rviz_plugins") + "/scripts/map_downloader.py";
