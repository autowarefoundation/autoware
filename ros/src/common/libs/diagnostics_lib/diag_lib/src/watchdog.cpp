...
watchdog::watchdog()
{
  ...
  for (YAML::const_iterator it = config.begin(); it != config.end(); ++it)
  {
    if (it->first.as<std::string>() != ros::this_node::getName())
    {
      ...
      diag_sub_[itr_node_name] =
          boost::make_shared<diag_subscriber>(itr_node_name, target_node["node_number"].as<int>());
  ...
  for (YAML::const_iterator it = config.begin(); it != config.end(); ++it)
  {
    ...
    for (const YAML::Node &error : target_node["errors"])
    {
      ...
      if ((category == LOW_SUBSCRIBE_RATE) || (category == LOW_PUBLISH_RATE) || (category == LOW_OPERATION_CYCLE))
      {
        ...
        line = node_name + "," + std::to_string(node_number) + "," + std::to_string(num) + "," + name + "," +
               std::to_string(category) + "," + description + "," + std::to_string(threashold) + "," + level + "\n";
      }
      else
      {
        line = node_name + "," + std::to_string(node_number) + "," + std::to_string(num) + "," + name + "," +
