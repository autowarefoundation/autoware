...
Watchdog::Watchdog() {
  ...
  for (YAML::const_iterator it = config.begin(); it != config.end(); ++it) {
    if (it->first.as<std::string>() != ros::this_node::getName()) {
      ...
      diag_sub_[itr_node_name] = boost::make_shared<DiagSubscriber>(
          itr_node_name, target_node["node_number"].as<int>());
      boost::shared_ptr<std::vector<DiagInfo>> target_node_watchdog_diag_info =
          boost::make_shared<std::vector<DiagInfo>>();
