...
diag_manager::diag_manager()
{
  ...
  try
  {
    ...
    {
      diag_info info(error["num"].as<int>(), error["name"].as<std::string>(), error["category"].as<int>(),
                     error["description"].as<std::string>());
      if ((info.category == LOW_SUBSCRIBE_RATE) || (info.category == LOW_PUBLISH_RATE) ||
          (info.category == LOW_OPERATION_CYCLE))
      {
        if (error["level"].as<std::string>() == "warn")
        {
          diag_info detail_info(error["num"].as<int>(), error["name"].as<std::string>(), error["category"].as<int>(),
                                error["description"].as<std::string>(), error["threshold"].as<double>(), LEVEL_WARN);
        ...
        {
          diag_info detail_info(error["num"].as<int>(), error["name"].as<std::string>(), error["category"].as<int>(),
                                error["description"].as<std::string>(), error["threshold"].as<double>(), LEVEL_ERROR);
          boost::shared_ptr<rate_checker> rate_checker_ptr =
              boost::make_shared<rate_checker>(RATE_CHECKER_BUFFER_LENGTH);
          checkers_[info.num] = rate_checker_ptr;
          diag_info_.push_back(detail_info);
  ...
  catch (...)
  {
    ...
    ROS_ERROR_STREAM("diag config file format was wrong. kill " + ros::this_node::getName() + ". Please check "
                     << error_code_config_path_);
    WRITE_LOG();
    std::exit(-1);
#else
    ROS_WARN_STREAM("diag config file format was wrong in " + ros::this_node::getName() + ". Please check "
                    << error_code_config_path_);
#endif
  }
    ...
    if (diag_info_itr->num == num)
    {
      if (diag_info_itr->threshold && diag_info_itr->level)
      {
        diag_info ret(diag_info_itr->num, diag_info_itr->name, diag_info_itr->category, diag_info_itr->description,
                      diag_info_itr->threshold.get(), diag_info_itr->level.get());
        return ret;
      }
      else
      {
...
bool diag_manager::check_error_code(int requested_error_number, std::vector<int> right_categories)
{
...
#ifdef STRICT_ERROR_CODE_CHECK
    ROS_ERROR_STREAM("error category : " << query_diag_info(requested_error_number)->category << " in "
                                         << ros::this_node::getName() << " does not match. Please check "
                                         << error_code_config_path_);
    WRITE_LOG();
    std::exit(-1);
#else
    ROS_WARN_STREAM("error category : " << query_diag_info(requested_error_number)->category << " in "
                                        << ros::this_node::getName() << " does not match. Please check "
                                        << error_code_config_path_);
#endif
    return false;
  ...
  {
    ROS_WARN_STREAM("error number : " << requested_error_number << " in " << ros::this_node::getName()
                                      << " does not exist. Check " << error_code_config_path_);
    return false;
  }
}
