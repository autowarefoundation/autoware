...
bool setup_cameras(std::vector<BGAPI::Camera*>& cameraObjects, std::string camera_pixel_format_str)
{
  ...
  for (unsigned int i = 0; i < cameraObjects.size(); i++)
  {
    ...
    for (int p = 0; p < pixellist.length; p++)
    {
      ...
      ROS_INFO("%c ID %x:%s | %d\n", (p == pixellist.current) ? '*' : ' ', pformat.iPixelFormat, (char*)pformat.sName,
               pformat.iPixelBits);
    ...
    if (pixel_formart_id_to_select != -1)
    {
      ...
      res = cameraObjects[i]->getPixelFormatDescription(formatlist.current, pixellist.array[pixel_formart_id_to_select],
                                                        &pformat);
      if (res != BGAPI_RESULT_OK)
      {
        ROS_INFO("BGAPI::Camera::getPixelFormatDesc error:%d\n", res);
...
int init_cameras(int system_count, vector<BGAPI::System*>* externppSystem, int* pCurrSystem, int& camera_count,
  ...
  for (systemIter = externppSystem->begin(); systemIter != externppSystem->end(); systemIter++)
  {
    ...
    for (cam = 0; cam < count; cam++)
    {
      ...
      printf("%d select Camera %d of system %d - %s SN: %s\n", camera_count, cam,
             (int)(systemIter - externppSystem->begin()), cameradeviceinfo.modelName, cameradeviceinfo.serialNumber);
