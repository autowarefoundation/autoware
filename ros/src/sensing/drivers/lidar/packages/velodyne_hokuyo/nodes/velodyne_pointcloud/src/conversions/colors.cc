...
namespace velodyne_pointcloud
{
  ...
  RingColors::RingColors(ros::NodeHandle node, ros::NodeHandle private_nh)
  {
    ...
    output_ =
      node.advertise<sensor_msgs::PointCloud2>("/vh/velodyne_rings", 10);

    // subscribe to VelodyneScan packets
    std::string topic = "/vh/velodyne_points";
    private_nh.getParam("velodyne_topic", topic);
    input_ =
      node.subscribe(topic, 10,
