...
void trans_depth_points_to_image_points(Scan_points_dataset* scan_points_dataset,
                                        Image_points_dataset* image_points_dataset)
{
  ...
  for (i = 0; i < (int)scan_points_dataset->scan_points.x.size(); i++)
  {
    ...
    if (camera_z >= 0.0)
    {
       ...
       */
      image_points_dataset->image_points.x.push_back((camera_x * cameraMat.at<double>(0, 0) / camera_z) +
                                                     cameraMat.at<double>(0, 2));
      image_points_dataset->image_points.y.push_back((camera_y * cameraMat.at<double>(1, 1) / camera_z) +
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
  ...
  //    msg->angle_min * 180 / 3.141592, msg->angle_max * 180 / 3.141592, msg->angle_increment * 180 / 3.141592,
  ...
  for (i = 0; i < (int)msg->ranges.size(); i++)
  {
    scan_points_dataset.scan_points.x.at(i) =
        msg->ranges.at(i) * sin(msg->angle_min + msg->angle_increment * i);  // unit of length is meter
    scan_points_dataset.scan_points.y.at(i) = 0;  // unit of length is meter
    scan_points_dataset.scan_points.z.at(i) =
        msg->ranges.at(i) * cos(msg->angle_min + msg->angle_increment * i);  // unit of length is meter
  ...
  for (i = 0; i < (int)image_points_dataset.image_points.x.size(); i++)
  {
    ...
    scan_image.distance[(int)image_points_dataset.image_points.x.at(i) * imageSize.height +
                        (int)image_points_dataset.image_points.y.at(i)] = image_points_dataset.distance.at(i);

    if (!msg->intensities.empty())
    {
      scan_image.intensity[(int)image_points_dataset.image_points.x.at(i) * imageSize.height +
                           (int)image_points_dataset.image_points.y.at(i)] = image_points_dataset.intensity.at(i);
    }
