...
## Automaic Camera intrinsic calibration (beta)

The intrinsic parameters can be also automatically generated by `cameracalibrator_automatic` node based on Structure From Motion (SFM) technique. This node collects images the constant cycle trigger or the delta pose trigger and finally execute SFM with collected images.

### How to launch
1. (When using "colmap" mode) Install colmap:\
https://colmap.github.io/install.html
1. In a sourced terminal:\
`rosrun autoware_camera_lidar_calibrator cameracalibrator_automatic.py image_raw:=<image_topic>`
1. Play a rosbag or stream from a camera in the selected topic name.
1. Move your car in a rosbag or real world to collect image.
1. A file will be saved in your home directory with the name `YYYYmmdd_HHMM_autoware_camera_calibration.yaml`.
...
---|---|---|---|
 `calibration_method` | *string* | `colmap` | Select SFM method for estimating intrinsic parameters, currently `colmap` mode  can be used |
 `trigger_method` | *string* | `transform` | Select image collecting method, `transform` (delta pose trigger mode) or `timer` (constant cycle trigger mode) |
 `collect_image_number` | *double* | `50` | The number of images to solve SFM |
 `parent_frame` | *string* | `world` | (`transform` mode) TF parent frame name for calculating delta pose |
 `child_frame` | *string* | `base_link` | (`transform` mode) TF child frame name for calculating delta pose |
 `delta_distance` | *double* | `5.0` | (`transform` mode) Distance trigger to take a shot [m] |
 `delta_rotation` | *double* | `3.0` |  (`transform` mode) Yaw rotation trigger to take a shot [deg] |
 `delta_time` | *double* | `1.0` |  (`timer` mode) Time trigger to take a shot [s] |
