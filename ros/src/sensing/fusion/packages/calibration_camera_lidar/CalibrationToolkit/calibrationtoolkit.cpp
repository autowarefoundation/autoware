...
CalibrationToolkitBase::CalibrationToolkitBase(QWidget *parent) : QWidget(parent)
{
...
CalibrateCameraBase::CalibrateCameraBase(QWidget *parent) : CalibrationToolkitBase(parent)
{
...
CalibrateCameraChessboardBase::CalibrateCameraChessboardBase(cv::Size2f patternSize, cv::Size2i patternNum,
                                                             QWidget *parent)
...
CalibrateCameraChessboardROS::CalibrateCameraChessboardROS(QString topic, u_int32_t queueSize, int interval,
...
bool CalibrateCameraChessboardROS::refreshImage()
{
  ...
  cameratimestamp = QTime::fromMSecsSinceStartOfDay((msg->header.stamp.sec % (24 * 60 * 60)) * 1000 +
...
bool CalibrateCameraChessboardROS::grabCalibData()
{
  ...
  cv::cornerSubPix(calibimage, grid2dpoint, cv::Size(11, 11), cv::Size(-1, -1),
...
CalibrateCameraVelodyneChessboardBase::CalibrateCameraVelodyneChessboardBase(float maxRange, cv::Size2f patternSize,
...
void CalibrateCameraVelodyneChessboardBase::extractionResultSlot(pcl::PointCloud<pcl::PointXYZI>::Ptr extraction,
...
double calibrationCameraVelodyneChessboardObjectiveFunc(const std::vector<double> &x, std::vector<double> &grad,
                                                        void *data)
{
  CalibrateCameraVelodyneChessboardBase::CameraVelodyneCalibrationData *calibdata =
      reinterpret_cast<CalibrateCameraVelodyneChessboardBase::CameraVelodyneCalibrationData *>(data);

  Eigen::Matrix3d rotation;
  rotation = Eigen::AngleAxisd(x[2], Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(x[1], Eigen::Vector3d::UnitY()) *
...
double calibrationCameraVelodyneChessboardRotationalObjectiveFunc(const std::vector<double> &x,
...
double calibrationCameraVelodyneChessboardTranslationalObjectiveFunc(const std::vector<double> &x,
                                                                     std::vector<double> &grad, void *data)
{
  CalibrateCameraVelodyneChessboardBase::CameraVelodyneCalibrationData *calibdata =
      reinterpret_cast<CalibrateCameraVelodyneChessboardBase::CameraVelodyneCalibrationData *>(data);
...
bool CalibrateCameraVelodyneChessboardBase::calibrateSensor()
{
  ...
  calibrationrotationalerror = cv::norm(calibrationdata.velodynenormals -
                                        calibrationdata.chessboardnormals * calibrationdata.rotationresult.t());
  ...
  Eigen::Matrix3d rotation;
  rotation = Eigen::AngleAxisd(xx[2], Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(xx[1], Eigen::Vector3d::UnitY()) *
    ...
    for (i = 0; i < n; i++)
    {
      ...
      for (j = 0; j < m; j++)
      {
        ...
        tmpvelodynetable->setItem(
            j, 3, new QTableWidgetItem(QString("%1").arg(calibvelodynespoints[i]->points[j].intensity)));
      }
      ...
      connect(planeextraction, SIGNAL(extractionResultSignal(pcl::PointCloud<pcl::PointXYZI>::Ptr, cv::Mat, int)), this,
              SLOT(extractionResultSlot(pcl::PointCloud<pcl::PointXYZI>::Ptr, cv::Mat, int)));
...
CalibrateCameraVelodyneChessboardROS::CalibrateCameraVelodyneChessboardROS(
    QString cameraTopic, u_int32_t cameraQueueSize, int cameraInterval, QString velodyneTopic,
    u_int32_t velodyneQueueSize, int velodyneInterval, float maxRange, cv::Size2f patternSize, cv::Size2i patternNum,
    QWidget *parent)
...
bool CalibrateCameraVelodyneChessboardROS::refreshVelodyne()
{
  ...
  velodynetimestamp = QTime::fromMSecsSinceStartOfDay((msg->header.stamp.sec % (24 * 60 * 60)) * 1000 +
                                                      msg->header.stamp.nsec / 1000000);
  calibvelodyne = msg;
...
CalibrateCameraLidarChessboardBase::CalibrateCameraLidarChessboardBase(float maxRange, cv::Size2f patternSize,
                                                                       cv::Size2i patternNum, QWidget *parent)
  : CalibrateCameraChessboardBase(patternSize, patternNum, parent)
{
  maxrange = maxRange;
...
double calibrationCameraLidarChessboardObjectiveFunc(const std::vector<double> &x, std::vector<double> &grad,
                                                     void *data)
{
  CalibrateCameraLidarChessboardBase::CameraLidarCalibrationData *calibdata =
      reinterpret_cast<CalibrateCameraLidarChessboardBase::CameraLidarCalibrationData *>(data);
...
bool CalibrateCameraLidarChessboardBase::loadCalibResult(cv::FileStorage &fs)
{
  ...
  if (button == QMessageBox::Yes)
  {
    ...
    for (i = 0; i < n; i++)
    {
      ...
      connect(pointsextraction, SIGNAL(extractionResultSignal(QVector<QPointF>, int)), this,
...
CalibrateCameraLidarChessboardROS::CalibrateCameraLidarChessboardROS(QString cameraTopic, u_int32_t cameraQueueSize,
...
bool CalibrateCameraLidarChessboardROS::refreshLidar()
{
  ...
  lidartimestamp = QTime::fromMSecsSinceStartOfDay((msg->header.stamp.sec % (24 * 60 * 60)) * 1000 +
                                                   msg->header.stamp.nsec / 1000000);
  caliblidar = msg;
...
bool CalibrateCameraLidarChessboardROS::grabCalibData()
{
  ...
  PointsExtractor *pointsextraction =
      new PointsExtractor(caliblidar, caliblidarspoints.size() - 1, caliblidarviewer->imagesize,
