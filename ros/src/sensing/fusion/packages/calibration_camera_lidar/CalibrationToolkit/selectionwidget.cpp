...
PlaneExtractor::PlaneExtractor(sensor_msgs::PointCloud2ConstPtr velodynePoints, int id, double neighborRadius,
  ...
  : GLViewer(parent)
{
  ...
  connect(this, SIGNAL(mousePositionSignal(QMouseEvent *, CAMERAPARAMETERS *)), this,
          SLOT(mousePositionSlot(QMouseEvent *, CAMERAPARAMETERS *)));
}

PlaneExtractor::PlaneExtractor(pcl::PointCloud<pcl::PointXYZI>::Ptr velodynePoints, int id, double neighborRadius,
                               double distanceThreshold, QWidget *parent)
  : GLViewer(parent)
{
  pointsid = id;
...
void PlaneExtractor::mousePositionSlot(QMouseEvent *event, CAMERAPARAMETERS *parameters)
{
  ...
  gluUnProject(tmpx, iViewPort[3] - tmpy - 1, 1, dModelMatrix, dProjMatrix, iViewPort, &(tmpd[0]), &(tmpd[1]),
               &(tmpd[2]));
  dir(0) = tmpd[0];
  dir(1) = tmpd[1];
  dir(2) = tmpd[2];
...
void PlaneExtractor::drawMouse(Eigen::Vector3d point, Eigen::Vector3d norm, Eigen::Vector3d direction,
                               Eigen::Vector3d nearestpoint)
{
  this->makeCurrent();
  glNewList(mousedisplaylist, GL_COMPILE);
...
PointsExtractor::PointsExtractor(sensor_msgs::LaserScanConstPtr lidarPoints, int id, int imageSize, double maxRange,
                                 double gridSize)
{
