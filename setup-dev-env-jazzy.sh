...
# Usage: setup-dev-env-jazzy.sh <ros2_installation_type('core' or 'universe')> [-y] [-v] [--no-nvidia]
...
print_help() {
    echo "Usage: setup-dev-env-jazzy.sh [OPTIONS]"
    echo "Options:"
...
# Load env - Modified to use amd64-jazzy.env
...
for env_name in $(sed -e "s/^\s*//" -e "/^#/d" -e "s/=.*//" <amd64-jazzy.env); do
...
if ! (python3 -m pip --version >/dev/null 2>&1); then
    sudo apt-get -y update
    sudo apt-get -y install python3-pip python3-venv python3-setuptools
fi
